...
|---|-------------|--------|--------|
| 1 | Unit suite | `scripts/tests/run.php` (`*Test.php`, 57 files at the frozen SHA) | built, executed in CI, executed on `720625b47f`: 3,421/3,422, RED on one letterhead assertion (M13_REVIEW Part II B2). Audit round on the merged tree (`766c0df`, where PR #65 flipped that assertion per the Owner's full-colour decision): 3,426/3,426 GREEN, and green in CI on `766c0df` independently (evidence `15`) |
