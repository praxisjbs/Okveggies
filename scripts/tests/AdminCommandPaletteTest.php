...
okv_test_eq(['g', 'd'], $commands[0]['shortcut'], 'Dashboard keeps its canonical direct shortcut');
okv_test_eq(['g', 'o'], $commands[1]['shortcut'], 'Orders keeps its canonical direct shortcut');
...
okv_test_ok(str_contains($paletteJs, 'if (event.repeat) { return; }'), 'repeated keydown events cannot repeat a shortcut action');
okv_test_ok(str_contains($paletteJs, "closest('input, textarea, select, [contenteditable]"), 'direct shortcuts protect every agreed editable control');
okv_test_ok(str_contains($paletteJs, "pendingPrefix = 'g'")
    && str_contains($paletteJs, "shortcutCommands['g ' + key]")
    && str_contains($paletteJs, '}, 1500);'), 'the Go to sequence uses the agreed prefix and 1.5 second timeout');
okv_test_ok(str_contains($paletteJs, 'document.contains(command)')
    && str_contains($paletteJs, 'data-command-shortcut'), 'direct navigation can activate only a permitted server-rendered command');
okv_test_ok(str_contains($paletteJs, "announce('You are already on '")
