...
okv_test_ok(str_contains($page, "header('X-Robots-Tag: noindex, nofollow')"), 'error responses reinforce noindex through an HTTP header');
...
okv_test_ok(str_contains($nav, "'content_slugs' => ['about', 'how-it-works', 'faq', 'terms', 'privacy', 'delivery-policy']"), 'footer content order selects fixed slugs without duplicating labels or paths');
okv_test_ok(!str_contains($nav, "'href' => '/faq'"), 'footer configuration does not compete with the canonical page-to-path registry');
...
okv_test_ok(str_contains($rewrites, '^(our-story|how-it-works|faq|terms|privacy|delivery-policy)/$'), 'trailing-slash content variants redirect to the slashless form');
