...
if curl -fsS -o /dev/null --max-time 5 "$BASE/index.php" 2>/dev/null; then
  for suite in \
    admin_dashboard_http_test admin_notifications_http_test customer_notifications_http_test cancellation_http_test contact_http_test contact_admin_http_test \
