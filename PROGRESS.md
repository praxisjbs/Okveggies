...
**Kitchen Run off-shop items fixed end to end, built on branch `arena/01a0cf85-okveggies` (23 September 2026), branched from `arena/01a0cf32-okveggies`.** "The Kitchen Run option for adding items that are not in the shop is not working." Four root causes, all evidenced with a jsdom driver against the real minified bundle and a payload capture run through the real server pipeline. First, the JavaScript numbered rows across both halves of the items step, the hidden one included, so in "Type my list", "Upload list" and "Already priced" the first visible row posted as `items[2]`, labelled "Item 3", the next as `items[3]`, "Item 4", and a mode switch never renumbered; every server refusal that named a line missed the row, and the line cap was two lines short before the customer typed anything. Second, the page rendered its hidden section's fields enabled, so without JavaScript a catalogue-mode form posted the hidden rows' empty `items[0]` values over the visible ones: a pure shop list was refused for the missing quantity it had just typed, and a mixed list lost its off-shop line to a key collision. Third, the line-scoped message templates read "Give line :line a quantity.", so a code that lost its number rendered "Give line this a quantity." (the one red unit assertion). Fourth, the server numbered lines after dropping the blank rows a form posts, so a fault after a blank row skipped a number and the refusal no longer matched the label. Adjacent, the upload mode demanded a file on the server even when the customer had typed lines, while the client let that through. Six owner decisions, all recommendations accepted: the active section numbers from zero, with catalogue continuing the typed rows after the shop rows and the cap counting only rows that post; server line numbers count posted slots including blanks, so "line 3" always matches "Item 3"; hidden sections are disabled server-side, so hidden equals disabled equals not posted; the numberless line-scoped message reads "Give this line a quantity."; an upload needs a file only when it has no lines; fractional quantities on non-decimal units keep the existing kitchen-run behaviour. What changed: `assets/js/kitchen-runs.js` now numbers only the rows of the half of the step the chosen mode is using and renumbers on every mode switch, with the bundle regenerated by the repository build (the build also re-mangled `admin-content.min.js` by identifier names only, proving the committed file had drifted from the lockfile-pinned esbuild; semantically identical); `kitchen-runs.php` numbers the off-shop rows after the shop rows and passes a disabled flag to both row components, which disable their fields when their section is not the one the mode shows; `KitchenRuns` gained `rowIsBlank()`, `validateSubmission()` now counts posted slots for the line numbers and the real lines for the cap, and the message templates fill "line N" or "this line"; `KitchenRunWorkflow` moved the attachment requirement from "always in upload mode" to "upload mode with no lines to read"; the controller's posted-items parser keeps the blank slots in their positions instead of dropping them, so the server numbers what the screen shows. Tests: `KitchenRunsTest.php` extended for the blank-slot numbering, the all-blank refusal, the cap that counts real lines, and `rowIsBlank()`; `kitchen_runs_db_test.php` for an upload with lines and no file, with neither, and with a file and no lines; `kitchen_runs_http_test.php` for the blank-slot `line_name:3` over the route, the no-JavaScript mixed and pure shop posts, and the upload-with-lines acceptance; and `kitchen_runs_visual_test.mjs` for the opening row posting as `items[0]` labelled Item 1, the mode round trip keeping the numbers honest, and the no-JavaScript mixed list. The unit suite is green at 4,254 assertions, up from 4,245 with one failing, the strict lint is clean on every touched file, the brand gate passes, and the build is reproducible. The database, HTTP and browser suites are written for the release gate, which has the MySQL 8 box and Chromium; this container has neither, and neither did it need a schema change.
