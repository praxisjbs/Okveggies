...
**Kitchen Run custom items fixed: the dead "Add item" button, row reordering, and the no-JavaScript fallback, built on branch `arena/01a0cf32-okveggies` (23 September 2026).** A customer picking "Type my list" (or "Upload list" or "Already priced") could not build a list. The "Add item" button sat beside its rows container rather than inside it, so `closest('[data-kr-rows]')` found nothing and a press added no row at all; a customer who needed four items saw one line go, with no error and no feedback. The typed rows had no way to reorder, the shop picker's "Anything not in the shop?" block had no per-line note, the unit dropdown offered retired units the server then refused, and a submission whose JavaScript failed left the wizard collapsed with nothing a person could send. The server is now the single authority and the page follows it: `KitchenRuns::validateSubmission()` returns a row-specific code (`line_name:N`, `line_quantity:N`, `line_unit:N`, `line_price:N`, and `unit_not_active:N` from the workflow) with the line number filled into the sentence the customer reads, so a refusal says "Give line 2 an item name." rather than something vague; the storefront form offers only active units, carries its line cap from the server as `data-max-lines` (one limit, not two that can drift), prefills the first deliverable day for the no-JavaScript post, and a `<noscript>` block reveals the whole wizard as one flat, sendable form; the row component gained up/down reorder buttons and an optional per-line note; and the JavaScript driver's add-host resolution, the root cause, was rewritten to walk up to the nearest block that holds a rows container, with `moveRow()` and `updateReorderButtons()` behind the same renumbering the server reads. Tests: `KitchenRunsTest.php` extended for the row-specific codes, their sentences, the `:line` fallback and the `data-max-lines` wiring; `kitchen_runs_db_test.php` and `kitchen_runs_http_test.php` extended for a typed list end to end (one item, several in order, a fractional quantity, catalogue and typed lines side by side, a hostile item name stored verbatim, a retired unit refused, each row-specific refusal, and a plain no-fetch post answered with a redirect rather than JSON); and a new browser suite `kitchen_runs_visual_test.mjs` drives add, reorder, remove and send at 390px and 1440px and then the same send with JavaScript off, wired into both `run_all.sh` and the release gate. The dead-button fix is the point of the milestone: it was invisible to every server test, because the server never saw rows that were never added, and only a browser that presses the button proves it now works.

**Dispatched and delivered emails made reliable, built on branch `arena/01a0cf0d-okveggies` (23 September 2026).** A stage email that never arrived used to fail five quiet ways at once, found by tracing the required path `api/v1/orders.php` action `transition` through `OrderLifecycle::transition()` and `Notifications::announceStage()` to `Mail::send()`, `notification_deliveries`, SMTP, cron and resend before changing anything. First, `announceStage()` had no idempotency of its own, so a repeated transition, a page refresh or two concurrent forms each wrote another notification and sent another email. Second, the trail link in the email came from `orderContext()`, which reuses the stored token or falls back to an order id, and an order id link 404s for a guest on any other device, so the guest's email carried a link that did not open. Third, an order with no usable address produced empty recipients, `send()` returned 0, and the stage was never recorded: no notification, no delivery row, no resend, nothing for the team. Fourth, the parent notification read `sent` whenever any send succeeded, so a failed email beside a landed in-app copy looked finished, and resend never recomputed the parent, so a recovered email left the notification reading failed. Fifth, an SMTP failure was contained correctly (the announcement runs after the commit, so the transition stands) but invisibly: the reason was classified, yet no staff alert existed and Order 360 showed a failure nobody had been told about. Six Owner decisions, all recommendations accepted: (1) idempotency lives in the dispatcher, one announcement per event and order, first write wins; (2) the parent reads `partial` when channels disagree, never `sent` on a half landing; (3) every stage email mints a fresh no-login share token at send time, so the trail link opens for a guest without an account; (4) a missing address is a recorded failed delivery with the fixed sentence, with the in-app copy still landing for an account holder; (5) resend re-reads the order's current address, so correcting the address then pressing Send it again works, and recomputes the parent; (6) every email failure raises one staff alert, on the bell and by email, to colleagues holding `orders.view`. What changed: `Notifications` grew the `admin_stage_email_failed` event and its six tokens, the rewritten `announceStage()` with its guard, `stageEmailOf()`, `recordStageWithoutEmail()`, `alertStageEmailFailed()` and `stageLabel()`, the `STATUS_PARTIAL` summary with `summariseDelivery()` and `refreshNotificationStatus()`, and the address re-resolve in `resend()`; `OrderTrail` grew `issueForOrder()`, a platform share issuance that never throws; `Mail` grew `maskAddress()`, masks addresses in `error_log`, and sets Reply-To to the support inbox; `AdminNotifications::EVENTS` admits the failure alert behind `orders.view`; `SettingsEditor::sampleTokens()` previews the new tokens; migration `060` seeds the alert template with `INSERT IGNORE`. The controller was not touched: it still decides only whether the transition committed. One deliberate shape: the bell carries one alert per order, naming the first stage that failed, because Order 360 lists every failed email beside its own resend, so the bell points at the order rather than repeating the ledger. Tests: new `StageNotificationTest.php` (unit), new `stage_notification_db_test.php` (MySQL 8: the full journey, skipped and invalid moves, repeats and a stale race, guests, missing and invalid addresses, mail server on port 1, correct then resend) and new `stage_notification_http_test.php` (the real controller over HTTP, the guards, the no-login link fetched as a signed-out visitor, Order 360 showing the failure and the resend), plus a scoping fix to the cancellation unit test whose whole-file `alreadyAnnounced` count the new guard tripped. The unit suite is green at 4,185 assertions, up from 4,076; the strict-parser lint is clean on all 333 shipped PHP files with a negative control proving it bites; the brand gate passes. The database and HTTP suites are written for the release gate, which has the MySQL 8 box and the mail sink. **Deployment:** the deploy applies migration `060` automatically. Nothing else is needed: recipients are read at send time, share tokens are minted at send time, and the alert starts reaching the team the moment the template lands. **Left for a person with a live site:** `bash scripts/verify.sh` against the deployed base, and the five minute cron endpoint check in `docs/DEPLOYMENT.md`, because stage emails are synchronous and this work changes nothing cron sends, but the gate should still see `CRON OK`.

**Cancelling an order now tells the team, built on branch `arena/01a0ceca-okveggies` (23 September 2026).** A cancellation wrote one notification and sent one email, both to the customer. `Notifications::announceCancellation()` fired a single event, `order_cancelled`, addressed to `orderContext()['recipients']`, which `customerRecipients()` fills from the order's account or checkout address and nothing else. There was no staff-audience cancellation event in `Notifications::EVENTS`, no `admin_*` cancellation template in any migration from `006` to `058`, and no cancellation entry in `AdminNotifications::EVENTS`, so the bell's permission boundary could not have admitted one even if a row had existed. PRD Section 15 listed the admin alerts and cancellation was not among them, which is why M6 never built it. The result: a crate that stopped being work still looked like work to everybody who had not opened that exact order, and a refund left needing a person by hand was told to the customer and not to the team. Six Owner decisions, all recommendations accepted: (1) one rule for both paths, so a staff cancellation alerts every authorised colleague including the person who pressed the button, and the bell stays a complete record; (2) the alert is gated by `orders.view`, the same permission that gates Order 360, which is where the alert links, so no new permission and no new boundary; (3) the message carries the position of the money in words and no naira figure, because the amounts are on Order 360 behind the same gate and a failed refund already reaches payments people through `refund_failed` with its figure attached; (4) idempotency lives in the dispatcher, not only in the controller, as a check for an existing notification row on that event and order; (5) a failed send records a sanitised reason category rather than the driver's own text, which can carry the SMTP host and account name into a column `admin/orders.php` prints; (6) retry stays the existing manual resend behind `notifications.resend`, with cron unchanged, so the alert is sent inline and immediately rather than queued. What changed: `Notifications` grew the `admin_order_cancelled` event, its seven tokens, `alreadyAnnounced()`, and three pure helpers (`cancellationSourceLine()`, `cancellationReasonLine()`, `staffRefundStateLine()`), and `announceCancellation()` now reads the committed `order_cancellations` row rather than trusting the caller, so no row means nothing is announced; `AdminNotifications::EVENTS` admits the event behind `orders.view`; `Mail` grew `classifyFailure()` and `lastError()` and the dispatcher's three `recordAttempt()` calls now store the category; `SettingsEditor::sampleTokens()` previews the new tokens from the same helpers that build the real message; migration `059` seeds the words with `INSERT IGNORE`, so a re-run cannot overwrite an Owner's edit. The controller was not touched: it decides only whether the cancellation committed, and everything about recipients, addresses and wording stays in the notification layer. PRD Section 15 now carries the decision. Tests: new `CancellationNotificationTest.php` (unit) and `cancellation_notification_db_test.php` (MySQL 8, auto-registered by the release gate glob), extended `AdminNotificationsTest.php`, `cancellation_http_test.php` and `admin_notifications_http_test.php`. The unit suite is green at 4,076 assertions, up from 3,683, run under a PHP 8.3.8 CLI installed in this container because it ships without one; `php -l` is clean on all 332 shipped PHP files; the brand gate passes; `npm run build` produces no change to the committed stylesheet and no JavaScript was touched. The database and HTTP suites are written and run in the release gate, which needs the MySQL 8 box and the mail sink. **Deployment:** the deploy applies migration `059` automatically. Nothing else is needed: the recipients are read at send time, so the alert starts reaching the team the moment the template lands. **Discovered and deferred:** two things this work turned up and did not change. `refund_failed` is sent with the whole customer order context, so `Mail::ctaFromVars()` puts a "Follow your order" button on a staff email instead of the admin link, the same wart the new alert was built to avoid; and `npm run build` rewrites `assets/js/admin-content.min.js` by one variable name against the committed bundle, a minifier drift with no source change behind it. Both are cosmetic and both are outside this issue.

**Phone and email login repaired, and one person is now one identity, built on branch `arena/01a0ce74-okveggies` (23 September 2026).** Sign-in by phone was failing for part of the team, and duplicate email or phone details across customer and staff accounts could create two rows for one person. The root cause was that only two of the six identity write paths stored canonical values. Registration and guest checkout stored the email lower case and the phone in E.164 (the work of milestone M1 Part 2), but staff creation (`api/v1/users.php` `create`), first-Owner setup (`public/setup.php`), and staff-made customer creation (`StaffCustomers::validateNew` kept the email case) stored whatever was typed. Sign-in looks identifiers up in canonical form through `Auth::findByIdentifier`, so a staff member stored as `0803 123 4567` could never be found by typing any form of their number. Migration `008` had only repaired two strict phone patterns, so legacy rows with spaces, dashes, a `00` prefix, or a 10 digit national form stayed behind too, and the raw-string duplicate checks could not see cross-format collisions. The fix, with six Owner decisions recorded (all recommendations accepted): (1) One real person is one identity. Staff access is a role on the same row (`user_roles`), never a second row, so a team member who shops keeps their household or business account type plus their role; the staff directory, the staff notification audiences, the staff password reset scope, and the first-Owner gate are all role-based now, with the legacy staff account type still honoured. (2) Migration `058_canonical_user_identities.sql` lower-cases every stored email, canonicalises every parseable phone the way `Phone.php` does, and writes every colliding or unparseable row to the new `user_identity_conflicts` report table; when two rows canonicalise to the same email or phone it fails the run with a SIGNAL instead of guessing, so the Owner resolves the pair and re-runs. (3) `Auth` grew `canonicalEmail()`, `findIdentityConflict()` (field-specific, shared by every create and edit path), `userHasStaffRole()` and `isStaffUser()`; `findByIdentifier` no longer uses LIMIT 1 to hide a duplicate, it fails closed and logs a repair alarm when two rows share one identifier. Every create and edit path (registration, staff creation, customer creation, guest checkout, profile edit, first-Owner setup) now shares the one canonicalisation and the one conflict check, and catches the unique-index race of two identical creations landing at once, answering with the field that collided. Duplicate refusals name the field (`email` or `phone`) on every create and edit path, while login and password reset keep their generic non-revealing answers. Tests: new `IdentityTest.php` (unit), new `identity_db_test.php` and `identity_http_test.php` (registered automatically by the release gate globs), extended `PhoneTest.php` and `StaffCustomersTest.php`. The unit suite is green at 3,683 assertions (run under a WebAssembly PHP 8.3 in this container, which has no PHP binary), `php -l` is clean on all 330 shipped PHP files, and the brand gate passes. The database and HTTP suites run in the release gate, which needs the MySQL 8 box. **Deployment:** the deploy applies migration `058` automatically. If it stops on the identity conflict SIGNAL, read the report with `SELECT * FROM user_identity_conflicts ORDER BY kind, canonical_value;`, resolve each pair (correct the wrong detail, or merge the rows and re-point their orders), then run migrations again; `unparseable_phone` rows do not block the run and can be fixed afterwards. Switching a dual identity off (`users.set_status`) switches their customer account off with it, since it is one row.

**Admin catalogue settings: category and unit management modal, built on branch `arena/01a0cd6b-okveggies` (23 September 2026).** Added a settings button beside the Add a product button on the Products page (`admin/products.php`) with a config/cog icon. Opening the button presents a tabbed modal for managing product categories and units of measurement. In the Categories tab, staff with `products.edit` permission can add new categories (with auto-generated unique slugs) and edit existing categories (with slugs kept immutable to preserve active web links). In the Units of measurement tab, staff can add new units and edit existing units, with an explicit confirmation step required if decimal permissions or active status are modified for a unit currently in use by catalogue products. Backend CRUD and validation are encapsulated in `CatalogueSettings`, wired to `api/v1/products.php` (`category_list`, `category_save`, `unit_list`, `unit_save`), and audited via `Audit::record`. Unit test suite passes with 3,556 assertions and brand checks remain all green.

**PR2 Fix 30, "Pick from shop" is a real picker and prices itself, built on branch `arena/01a0bfa7-okveggies` (20 September 2026).** The Owner picked "Pick from shop" on the Kitchen Runs page and got the same free-text rows as "Type my list": no product picker, no shop prices, and a wait for a quote that was never needed, because the backend could already store a product on every line and read its price from the products table. Step 2 now has a shop section (one dropdown of the shop's products with their prices per unit, a quantity, a live line total, "Add a line" under the rows, a running total, and a collapsible "Anything not in the shop?" block for typed lines) and a typed section for the other three modes, and only the section the chosen mode needs is ever visible or posted. A list picked entirely from the shop is quoted the moment it is sent (PRD 8.3): the request lands as Quoted at today's shop prices, the customer only approves, and the team gets a dedicated `admin_kitchen_run_shop_priced` alert that says check and convert rather than price this. One typed line, an open budget, or a staff-typed list still waits for a person. The unit suite is green at 3,455 assertions (run under a WebAssembly PHP 8.3 in this container, which has no PHP binary) and the brand gate passes; the branch is then merged with `main` at `72f59b8` (the homepage hero PR), with the two conflicts resolved by keeping both Current focus entries and rebuilding the compiled stylesheet from source, and the merged tree re-verified: 3,485 of 3,485 assertions green, all shipped PHP files parse clean, brand gate green. The database and HTTP suites for this change are written and run in the release gate, which needs the MySQL 8 box.

**Homepage hero redesign, merge authorised by the Owner on `arena/01a0bf68-okveggies` (20 September 2026).** The approved produce photograph now fills a full viewport below the unchanged header, with a full forest wash, unboxed copy, the existing seal and tagline, and the existing CMS CTA labels and destinations. The exact approved heading and introduction take display precedence by the Owner's confirmation without rewriting stored CMS copy. The existing motion controller owns the cinematic image entrance, return-to-hero replay, 7% parallax, seal, words, hairline and CTAs; this hero has a final still state for reduced motion and no JavaScript. Local static gates and the browser template fixture pass. PHP-backed integration remains unverified locally. The Owner has now requested conflict resolution and merge; that authorisation is not a completion claim for M12, PR6 or PR9. See the session entries below.

**Deploy pipeline restored, built on branch `arena/01a0bf7e-okveggies` (20 September 2026).** Every deploy to production from the PR #57 merge onward (deploy runs #50 through #57) failed at the "Apply migrations on the server" step, even though the SFTP upload succeeded each time. The cause: PR #57 edited `migrations/003_reference_seed.sql`, a migration the production database had already applied. The Migrator records a SHA-256 checksum per applied migration in `schema_migrations` and refuses to run when an applied file drifts ("Refusing to skip 003_reference_seed: checksum changed"), so the web runner answered 500 and the deploy step's grep for `MIGRATE OK` failed, on every run after. Meanwhile production kept receiving fresh code with no migrations, which is why 051, 053 and 054 never applied and the shop pages blanked on the missing `products.source_region` column. The fix restores 003 byte for byte to the applied version (git blob `579432e`, verified against the last green deploy `f527ff4`); migration 051 already carries the Monday business decision to every database, fresh installs included, so nothing is lost. The next deploy sees 003 as OK, applies 051, 053 and 054, answers `MIGRATE OK`, and the verify step's shop smoke checks run again. House rule now stated plainly in the 051 header: migrations are append-only, and a change to an already-applied migration belongs in a new migration. This branch also returns the unit suite to green: `DocumentTest` now pins the full-colour lockup on the letterhead, the Owner's "logo harmony" decision, which had blocked every CI run since commit `1467fe7`.

**PR1, Legal Truth and Delivery Truth, built on branch `arena/01a0bd40-okveggies` (20 September 2026).** First of the 10 PRs in `docs/REMAINING_27_FIXES_8PR_PLAN.md`, closing fixes 1, 3, 14 and 18. Monday is now a business delivery day in the seed and in migration `051`; the cancellation asymmetry is settled with the Owner and recorded in `docs/CANCELLATION_ASYMMETRY_DECISION.md`, with `Cancellation::depositShare()` making a late cancellation cost the same share whichever way the order was paid; the legal attest workflow is proved page by page with tests, and a legal readiness panel on Page Copy states what is still owed; the M2 carry-forward seed assertions moved from regexes over SQL to a migrated database, and the empty category stays visible as Being sourced. The unit suite is green at 3,380 assertions and the brand gate passes. The database and HTTP suites for this PR are written and registered; they run in the release gate and CI, which have no MySQL 8 in this build container. The next parallel PRs are PR2 messaging, PR3 checkout and PR5 infrastructure.

**Milestone M11, Admin dashboard and analytics. Reviewed and merged on branch `M11-Admin-dashboard` (pull request 48) on 16 September 2026.** The analytics engine, layered dashboard permissions, command palette, keyboard shortcuts and notification bell are strong work. One cross-milestone defect was fixed on the merge: the branch was cut before M10 landed and reinvented the `admin_new_issue_report` staff alert with different token names, a re-defining migration and an uncalled duplicate sender, which would have broken M10's live alert and clobbered edited copy. M10's version now stands; M11 keeps only the genuinely new `admin_manual_payment_proof` template. The senior review is `docs/M11_REVIEW.md` and the delivery evidence is `docs/M11_HANDOVER.md`. The minor support-widget fix (pull request 46) also merged: the contact form's `form.action` was shadowed by its hidden `action` input, and the widget-less contact page never wired its submit handler.

**Milestone M10, Make It Right, completed on branch `M9-issue-report-make-it-right` on 10 September 2026.** Customer reporting, private photos, the staff queue, the three terminal outcomes, the signed-in customer outcome history, storefront trust signals, issue notifications, the full lifecycle test matrix and the deployment handover are complete. Refunds delegate to M5, account credit appends through M8 for an approved business, and replacement links an existing manual order for the same customer. Customer outcomes stay off the public token trail, while the authenticated owner sees every report and its current result. New reports alert active staff with `issues.view`; customers receive durable receipt and outcome messages, including the exact decline reason, while refund progress continues through M5. Migrations `045` through `048` apply cleanly and idempotently on MySQL 8; `049` remains free. The next product milestone is M11, Admin dashboard and analytics.
...
M6 needed the migrations (`018` to `021`) run, SMTP proved with "Send one to me" on the Notifications settings tab, and one order taken end to end on staging with a Paystack test key.
...
- [~] Carried forward from the audit, not M2 acceptance criteria. **Two of the three parts are now closed, on 20 September 2026 in PR1 (fix 18).** The seed assertions no longer regex the SQL files: `scripts/tests/reference_seed_db_test.php` asserts the 5 categories, the 4 units, the 24 products, the Garlic kilogramme correction and the delivery-day decisions against a migrated database, and it is registered in the runner and the release gate. An empty category stays visible and is labelled Being sourced (Owner decision, 20 September 2026), pinned by the same suite: a category with 0 active products is returned by `Catalogue::categories()` with `product_count` 0, which the homepage renders as Being sourced and the shop renders as its Still sourcing empty state. **Still open:** give products a real source region instead of the one site-wide setting, now planned as fix 17 in PR4 of `docs/REMAINING_27_FIXES_8PR_PLAN.md`. Paginating the shop grid, which used to open this line, was delivered on 1 Sep 2026 and is specified in PRD Section 5.6
...
- [x] A cancellation reaches the team and not only the customer (added 23 September 2026, was missing from the milestone and from the PRD Section 15 admin-alert list). One new staff event `admin_order_cancelled`, seeded by migration `059`, addressed at send time to active users holding `orders.view`, admitted to the bell and history behind the same permission, idempotent on the order, and carrying the order number, the customer, who cancelled it and why, the delivery date and where the money stands, with no naira figure in it. See the session log for 23 September 2026.
...
### M7. Kitchen Runs
- [x] Request flow, four input modes (catalogue, priced-by-us, priced-by-customer, upload), a list of up to 100 lines rather than one item
- [x] Already-priced confirm path; open-budget trust mode with admin deposit and optional cap
- [x] Admin quote workflow, a full line editor so an uploaded list can be transcribed; convert to order
- [x] A converted run is an ordinary order: address snapshot, trail token, payment rows, delivery schedule, day manifest
...
Pull request 36 was reviewed against the PRD and completed on
`claude/milestone-7-completion-2nlnib`, which starts by merging that branch so
the work it did keep is credited in the history.

**The one that matters.** `KitchenRuns::convert()` threw
`PDOException: Invalid parameter number` on every call. Its orders INSERT bound
`:total` twice, once for `subtotal_subunit` and once for `order_total_subunit`,
and `Database` sets `ATTR_EMULATE_PREPARES` to false, so MySQL rejects a named
placeholder used more than once. Convert-to-order, the headline acceptance
criterion, had never worked once. It shipped behind twenty green assertions
because the only conversion test called `convertAtomically()` with a callback
that threw, and that path returns before any write. A conversion test that never
converts is not a test. This is the second time this exact defect has reached a
branch: the M6 review found the same reused placeholder in the orders customer
filter. The lesson did not transfer because nothing in the suite made it
transfer, so now something does: the HTTP test loads both Kitchen Run screens,
every status filter and every start mode, and converts over the real route.

**Six rules that nothing called.** `validateSubmission()`, `allowedUpload()`,
`mayTransition()`, `quoteExpired()`, `canCustomerEdit()` and
`notificationEvents()` had forty passing assertions between them and not one
production call site. The upload one was the sharpest: it enforced the filename,
MIME and size whitelist, was fully tested, and the controller called
`Uploads::saveUploadedFile()` directly, so the tested rule guarded nothing. All
are wired now, and `KitchenRunsTest.php` opens with a guard that reads every
shipped PHP file and fails if any public rule on `KitchenRuns` has no call site
outside the tests. That guard immediately found `notificationEvents()`, which
genuinely had no honest home, so it is deleted rather than exempted.

**What the milestone had not built, against PRD Section 8.**

- **A list.** The customer form took exactly one item, with a legend reading
  "One item, add more by submitting another request if needed". A Kitchen Run is
  a list; that is the entire feature. It now takes up to 100 lines, added and
  removed by `assets/js/kitchen-runs.js`, with three server-rendered rows and a
  plain form post as the path with JavaScript off.
- **Two of the four ways in.** Already-priced (PRD 8.1 mode 4) was not on the
  form at all, and a mixed catalogue-plus-free-text list was impossible because
  catalogue hydration threw unless every line carried a product id.
- **Transcription.** An upload arrived as one placeholder line and the quote form
  could only edit lines that already existed, so a photographed list of fourteen
  items could never become fourteen lines. PRD 8.1 mode 3 could not happen. The
  quote form is a full line editor now: add, edit, remove.
- **A delivery address.** Conversion wrote no `order_addresses` row and no
  `order_trail_token_hash`, so a converted run reached the day manifest, the
  packing list, the invoice, the receipt and the order-placed email with no
  recipient, and the customer had no Order Trail to follow. Everything M6 built
  quietly failed for exactly these orders. The address is captured with the list
  now, and conversion calls the same `Checkout` writers a checkout order uses,
  so the two are indistinguishable downstream. Those four Checkout helpers were
  made public rather than copied.
- **Anybody being told.** Nothing notified anyone. Four events fire after commit
  now: the team on a new list (PRD Section 14 asks for this by name) and on an
  approval, the customer on a quote and on a decline.
- **A trail.** `kitchen_run_status_history` is append-only like every other
  lifecycle here, and `transition()` is the single gate that checks the state
  map, bumps the version and writes the line. Before this, nothing recorded who
  declined a request or when a customer approved one.

**Six more defects found while finishing it.**

1. A failed form post rendered a raw JSON body as the whole page and lost
   everything the customer had typed, because the form posted natively and
   `okv_error()` only speaks JSON. Plain posts now get a 303 back to the screen
   with an error code, and one map turns that code into the same sentence the
   fetch caller receives.
2. `convertAtomically()` returned `['ok' => false]` on failure and the controller
   immediately read `$result['already_converted']`, so an injected failure would
   have been reported to staff as a success. The wrapper existed only for
   test-only fault injection and is gone.
3. Conversion never checked the delivery day, so a Kitchen Run could be
   scheduled on a day the shop does not deliver. It goes through
   `Delivery::isEligible()` now, the same as checkout.
4. Hand-typed money went through `Money::toSubunit()`, which is deliberately
   forgiving because it also takes values we generated: it reads `abc` as 0,
   `-5` as minus five hundred kobo and `1e3` as thirteen naira. Every figure on
   a Kitchen Run is typed by a person, so a silently wrong price becomes a real
   mispriced order. `KitchenRuns::nairaToKobo()` refuses anything that is not
   plainly money, following the pattern `SettingsEditor` already uses.
5. PHPMailer had no `Timeout`, so its 300 second default meant an unreachable
   mail host held a customer's request for up to five minutes. This is
   pre-existing M6 code, but M7 puts it on the customer's own submit path, so
   `SMTP_TIMEOUT` was added, 10 seconds by default. It showed up because the
   HTTP test took two minutes a call before it was fixed.
6. Migration `022` wrapped its DDL in `START TRANSACTION` and `COMMIT`, which
   MySQL commits through anyway. It had not shipped, so the wrapper is gone
   rather than left to be copied.

**Decisions taken before any code was written.** Six questions, three options
each, answered by the owner: the delivery address is captured at submission;
list entry is repeating rows with a no-JavaScript fallback; staff get a full
line editor so uploads can be transcribed; all four notifications are sent;
staff may re-price a Quoted request in place, which clears the approval so a
customer never owns figures they have not seen; and a quote expires after
`kitchen_run_quote_days`, 7 by default and editable in Order Settings, checked
when the quote is read rather than by a cron job.

**Shape.** The domain class reached 1,321 lines, longer than anything else in
the repository. It splits the way `Settings` and `SettingsEditor` already do:
`KitchenRuns` holds the rules and the reads that every screen calls,
`KitchenRunWorkflow` is the only thing that changes a request.

**Verification.** Against MariaDB 10.11 built only from migrations. All 25
migrations applied to an empty database, then `022`, `023` and `024` re-applied
by hand to prove they are idempotent. 1,755 unit assertions, 89 database
assertions including a conversion checked line by line (order total, subtotal,
deposit, address snapshot, order items, both payment rows, status history,
delivery schedule, trail token, and the M6 notification context reading it
back), and 42 HTTP assertions over the real routes covering the method gate,
CSRF, RBAC, ownership, the stale-version guard, the attachment route as a
direct object reference, and both screens loading with every filter and every
start mode. An upload was pushed through the real endpoint, the bytes came back
byte-identical to the owner, a signed-out stranger got 404, and a `.php` file
posing as an image was refused on its sniffed type. `scripts/brand-check.sh`
green. `php -l` on every touched file.

**One caveat, stated plainly.** The database used here is MariaDB 10.11, because
no MySQL 8 build was installable in this environment. MariaDB accepts some
syntax MySQL 8 rejects, notably `ADD COLUMN IF NOT EXISTS`, so a green run here
is not by itself proof against MySQL 8. Migrations `023` and `024` deliberately
use only the `information_schema` guard plus a prepared `ALTER`, the pattern
`011` and `020` already use and which works on both, and `022` was corrected to
match. Still worth watching on the first production deploy.

**Still to do on the server.** Take one Kitchen Run end to end on staging:
request, quote, approve, convert, then check the order appears on the day
manifest with a recipient on it and that the customer's Order Trail link works.
The authenticated browser journey at 390px and 1440px is the one thing this
branch could not exercise, for the same reason the first attempt could not.

### 4 Sep 2026, M7 Kitchen Runs, the first attempt (pull request 36)

The entry below is the one written at the time, kept as it was. Read it against
the review above: the verification effort in it was real and careful, and it
still concluded that a feature which threw on every call was complete. That gap
is the whole lesson, and it is written up for the engineer in `docs/M7_REVIEW.md`.
...
- [x] **Task G, issue notifications:** every saved report alerts active staff who hold `issues.view` and sends the customer a receipt with the order, category, submitted date, description preview and private account link. Every terminal outcome sends the existing resolved message with its exact customer-facing reason; refunds also retain the M5 refund-status sequence. All three templates use the M6 dispatcher, remain editable and previewable in Settings, and record a durable failed delivery for retry without losing the report.
- [x] Tests: issue lifecycle
- [x] **Task H, lifecycle verification:** the complete open, taken and terminal state graph is covered, including every forbidden move and terminal replay. Service and HTTP tests refuse another customer's order and the first second outside the Lagos reporting window. Refund tests prove the exact approved amount reaches the linked M5 transaction and cannot exceed an immutable selected line. Real multipart tests reject wrong extensions, disguised content and oversized images. Public token tests withhold report state, words, outcomes, identifiers and photos. Guest, owning customer, other customer, Owner, Manager and partial custom roles are covered with POST, CSRF and permission gates on every write.
- [x] **Task I, finishing and handover:** all M10 criteria are closed, migrations `045` to `048` apply to fresh MySQL 8 and rerun with nothing pending, all M10 suites are part of the normal test gate, and the deployment smoke script covers the API, private customer order, staff queue and protected photo route. `docs/M10_HANDOVER.md` records features, permissions, migrations, verification, deployment checks and operating guidance. No work was deferred from M10; Apache or cPanel must still prove that `/migrations/` and `/docs/` are denied during deployment.
...
- [~] **Task F, M12 homepage acceptance.** The existing database-driven storefront has been audited rather than rebuilt. Published `home` fields now control the hero, promise and section headings with reviewed public fallbacks that never expose a draft. Featured combos respect the explicit featured flag; combos, categories and products have useful empty and isolated error states; all 5 active category routes remain usable, including a "Being sourced" state; and Shop, Combos and Kitchen Runs are clear routes in the page body. Homepage SEO fields now drive the title and description with canonical and Open Graph URL coverage. Content admin accepts a rights-cleared Homepage or Our Story photograph through POST, CSRF and `content.edit`, requires alt text, and creates randomised 640px, 960px and 1280px WebP variants under the protected uploads tree. No approved Track 2 photograph exists in the repository, so the public hero shows the agreed branded dependency state and the headline checkbox correctly remains open.
- [~] Our Story, How It Works, FAQ, Terms, Privacy, Delivery Policy (editable in admin). The renderer, editor and publishing workflow are complete, but Terms, Privacy and Delivery Policy remain unpublished until the client supplies and approves final legal copy. PR1 (20 September 2026) made the legal workflow provable: each of the three pages is tested to refuse publication without the client-copy attestation, to stay out of footer navigation and the sitemap while unpublished, to return the same branded noindex 404 on its public path, and to record the attested actor in the publish audit. A legal readiness panel on Page Copy names the owed pages plainly and disappears once all three are published, so its presence is itself the honest state.
...
- Business delivery days are Monday, Tuesday and Friday. Monday was opened to businesses as well as households on 3 September 2026; `003_reference_seed.sql` now seeds it and migration `051` carries the decision to databases seeded before the correction. Household days are unchanged: Monday, Wednesday, Thursday, Saturday.
- Cancellation after the cutoff is settled (Owner, 20 September 2026): the deposit share is kept whichever way the order was paid, the deposit taken or the same percentage of an order paid in full, never more than was paid, and everything above the share comes back. `docs/CANCELLATION_ASYMMETRY_DECISION.md` is the record.
- The remaining storefront scaffolds get the shared brand shell when their owning milestone builds them. M12 has now replaced the former `page.php` scaffold with the shared accessible content-page renderer.
...
- Remaining scaffolds continue to name their owning future work. M12 has now built `page.php`, the Page Copy half of `admin/content.php`, its write controller, preview and public discovery layer; the M9 Messages behaviour on the same screen remains intact.
...
### 23 Sep 2026, Kitchen Run custom items: the dead "Add item" button, reordering, and the no-JavaScript fallback

- **The report.** On the Kitchen Runs page, "Type my list", "Upload list" and "Already priced" all render free-text rows, and a customer could not build a list with them. The "Add item" button is a sibling of its `data-kr-rows` container, not a child, so the click handler's `closest('[data-kr-rows]')` returned null and a press added nothing: no row, no error, no feedback. A customer who needed four items sent one. Typed rows could not be reordered and the shop picker's "Anything not in the shop?" block had no per-line note. The unit dropdown listed retired units, so a customer could pick one and be refused at the server with a generic message. And with JavaScript off the wizard stayed collapsed on step 1, so a person whose script failed had no way to send anything.
- **Root cause and the shape of the fix.** The server is the authority and the page now follows it. `KitchenRuns::validateSubmission()` returns a row-specific code (`line_name:N`, `line_quantity:N`, `line_unit:N`, `line_price:N`, 1-based, keeping `invalid_line` only for a line that is not an array) and `message()` fills the `:line` placeholder, falling back to "this", so the customer reads "Give line 2 an item name." rather than a vague refusal. `KitchenRunWorkflow` gained `assertActiveUnits()`, which throws `unit_not_active:N` and is called from both `submissionLines()` and `quote()`, so a retired unit is refused the same way whichever door it comes through. The storefront offers only `is_active = 1` units, carries its cap from the server as `data-max-lines`, prefills the first deliverable day for the no-JavaScript post, and a `<noscript>` block reveals the whole wizard as one flat form with a note that lines save in the order they appear.
- **The JavaScript driver.** `addRowTo` resolved its host with `closest()`, which only walks ancestors and so never found the sibling container; the rewritten `rowsHostNear()` walks up to the nearest block that holds a `[data-kr-rows]`. Added `moveRow()` and `updateReorderButtons()` so a row moves one place inside its own container and the end buttons hide at the ends, all behind the same `renumber()` the server reads. `itemsProblem()` now treats a note-only row as touched, so a half-typed line is caught before it is sent. The row component gained up/down buttons (hidden until JavaScript runs) beside Remove and a visible optional per-line note. The admin panel loads all units, keeps the active list for new lines, and shows a retired unit as "(retired)" on a line that still names one, so a colleague can see why a past line will not re-quote.
- **Tests.** `KitchenRunsTest.php` extended (row-specific codes, the line-number sentences, the `:line` fallback, the `data-max-lines` wiring guard). `kitchen_runs_db_test.php` and `kitchen_runs_http_test.php` extended: a typed list end to end (one item, several in order, a fractional 2.500 quantity, catalogue and typed lines coexisting, a hostile item name stored verbatim), a retired unit refused, each row-specific refusal, and a plain no-fetch post answered with a redirect. New browser suite `kitchen_runs_visual_test.mjs` drives add, reorder, remove and send at 390px and 1440px and then the same send with JavaScript off; it is wired into `run_all.sh` and the release gate's browser section and completeness guard.
- **Run here.** `node --check` clean on the driver and the new suite, `npm run build` regenerated `kitchen-runs.min.js`, `brand-check.sh` green. The unit suite, the PHP database and HTTP suites and the browser suite need the MySQL 8 box, the seeded scratch server and Playwright's Chromium, none of which this build container has, so they are written for the release gate rather than claimed green here. The dead-button regression in particular is provable only in a browser, because the server never saw the rows that were never added.

### 23 Sep 2026, dispatched and delivered emails made reliable

- **The report.** Dispatch and delivery emails were unreliable: sometimes missing, sometimes doubled, sometimes addressed wrong, with no clear record of what went out. Reproduced on paper first by tracing the required path end to end, `api/v1/orders.php` action `transition` through `OrderLifecycle::transition()` and `Notifications::announceStage()` into `dispatch()`, `Mail::send()`, `notification_deliveries`, the SMTP settings, `Cron::run()` and `Notifications::resend()`, and by inspecting `STAGE_EVENTS` and every caller of the stage path. The transition itself was never the problem: it commits under its guard, writes one history row, and announces only on `code === 'transitioned'`, and it is the sole caller of `announceStage()`, which the new unit suite pins. Five defects sat behind it, all in the notification layer. (1) `announceStage()` had no idempotency check, so a repeated transition, a refresh or two concurrent forms each wrote another notification and sent another email. (2) The trail link came from `orderContext()`, which reuses the stored token or falls back to `order=`, and an order id link 404s for a guest on any other device, so guest emails carried a link that did not open. (3) No usable address meant empty recipients, `send()` returned 0, and the stage vanished: no notification, no delivery row, no resend, nothing for the team. (4) The parent read `sent` whenever any channel landed, so a failed email beside a landed in-app copy looked finished, and `resend()` never recomputed the parent. (5) An SMTP failure was contained but invisible: the announcement runs after the commit so the transition stands and the reason is classified, yet no staff alert existed and Order 360 showed a failure nobody had been told about. Two smaller findings rode along: `Mail` logged full customer addresses to `error_log`, and no Reply-To was set, so replies went to a sending mailbox nobody reads.
- **Six questions, six answers, all recommendations accepted.** Idempotency lives in the dispatcher, one announcement per event and order; the parent reads `partial` when channels disagree; every stage email mints a fresh no-login share token; a missing address is a recorded failure with the in-app copy still landing; resend re-reads the order's current address and recomputes the parent; every email failure alerts colleagues holding `orders.view`, on the bell and by email.
- **What was built.** `Notifications::announceStage()` rewritten: unknown stages and already announced pairs return before anything is minted or written, then a fresh token from `OrderTrail::issueForOrder()`, then the context, then either the recorded no-address path or the send, then a check of the email delivery row that raises `admin_stage_email_failed` when it failed. `recordStageWithoutEmail()` writes the notification with the rendered template, lands the in-app copy for an account holder, records the email as failed with `FAIL_NO_ADDRESS`, keeps what was typed on the row so staff can see the typo, and alerts the team. `summariseDelivery()` and `refreshNotificationStatus()` give the honest three way parent, `sent`, `partial`, `failed`, and `resend()` now re-resolves the address from the order, updates the row, refuses with `no_address` when there is still nowhere to send, and recomputes the parent after every attempt. `OrderTrail::issueForOrder()` is platform issuance into `order_trail_share_links`, validated, collision checked, never throws, and the email carries `token=` with the order id fallback only when minting failed. `Mail::maskAddress()` reduces logged addresses to first letter plus stars, `(no address)` or `(invalid address)`, and Reply-To is the support inbox setting when bootstrapped. `AdminNotifications::EVENTS` admits the failure alert behind `orders.view`, the same gate as the Order 360 page it links to. `SettingsEditor::sampleTokens()` previews the new tokens from the same helpers. Migration `060_stage_email_failure_alert.sql` seeds the alert template with `INSERT IGNORE` and ends with verification queries, so a re-run cannot overwrite an Owner edit. `api/v1/orders.php` untouched; `admin/orders.php` untouched, it already renders `deliveryState()`, which already says Not sent with the resend beside it.
- **Deliberate shapes.** The bell carries one alert per order, naming the first stage that failed, because Order 360 lists every failed email beside its own resend button; the bell points at the order rather than repeating the ledger. Cron is unchanged on purpose: stage emails send synchronously with `scheduled_at` NULL, and `flushDue()` only touches `queued` rows with a due time, so the cron pass can neither send nor double-send a stage email; verified by reading, and the release gate still checks `CRON OK` separately.
- **Tests.** New `scripts/tests/StageNotificationTest.php` (unit: the catalogue, the labels, the three way summary, the dispatcher wiring, the trail issuance, the bell boundary, the preview, the masking, the migration shape and token coverage, and the rule that the controller decides nothing). New `scripts/tests/stage_notification_db_test.php` for MySQL 8, auto-registered by the release gate glob: the full journey announcing every stage exactly once with the order number, name, delivery day and a token that resolves to the order, one share link per stage, history intact and attributed, skipped and invalid moves announcing nothing, repeats and a stale race writing nothing new, a guest announced by checkout address with a working no-login link, an invalid account address recording partial and a guest with nowhere to send recording failed, both alerting the team on the bell and by email with the `orders.view` boundary proved both ways, mail server on port 1 leaving the dispatch committed with a classified reason and a bell alert that still lands, resend on the same row at attempt 2 flipping the parent to sent, a refused third send, correct the address then send it again, and an unknown event sending nothing. New `scripts/tests/stage_notification_http_test.php`: the journey over the real controller, the trail link fetched as a signed-out visitor through the test base (never the APP_URL host), repeats, stale 409, backwards 422, customer 403, missing CSRF 419, GET 405, the guest move, the no-address move showing Not sent with Send it again on Order 360, the `no_address` refusal, the corrected resend, and the finished journey showing no failure. One existing test needed a scoping fix, not a revert: `CancellationNotificationTest.php` counted `alreadyAnnounced(` across the whole dispatcher file, and the new stage guard made it three, so it now counts each cancellation half by its own event name, which is the property it pins.
- **Run here.** The unit suite is green at 4,185 of 4,185, up from 4,076 before the change. This container has no PHP binary and no MySQL server, and the package mirror is unreachable, so the suite ran under a WebAssembly PHP 8.3 with the repo mounted, through a runner that lives outside the repository, and `php -l` was stood in for by a strict PHP 8.3 parser over all 333 shipped PHP files, clean, with a deliberately broken file proving the gate bites. `bash scripts/brand-check.sh` is green on all eight laws. Not run here: the two new database and HTTP suites, which need the MySQL 8 release box and the mail sink, and `bash scripts/verify.sh` plus the cron endpoint check, which need a live site. Those are the release gate's.
- **Deployment.** The deploy applies migration `060` automatically. Nothing else is needed: recipients are read at send time, share tokens are minted at send time, Reply-To falls back silently when the support inbox setting is empty, and the failure alert starts reaching the team the moment the template lands. No config change: SMTP host, port, encryption, user, from name, timeout and the five minute cron stay exactly as `docs/DEPLOYMENT.md` has them.

### 23 Sep 2026, cancelling an order did not tell the team

- **The gap.** `Notifications::announceCancellation()` sent one event, `order_cancelled`, to `orderContext()['recipients']`, which is `customerRecipients()` and nothing else. Both cancellation paths in `api/v1/orders.php` (`cancel_customer` at line 100, `cancel_staff` at line 135) called that one method, so the staff path told the customer and not the colleague. No staff-audience cancellation event existed in `Notifications::EVENTS`, no template existed in migrations `006`, `007`, `010`, `019`, `027`, `042`, `045` or `048`, and `AdminNotifications::EVENTS` had no entry, which is the permission boundary for the bell, the API and the history page. Traced both paths end to end before editing: form, POST and CSRF and `confirmed` gate, `orders.cancel` for staff, `OrderCancellation::cancel()` under a `FOR UPDATE` lock, the `already_cancelled` short-circuit, the commit, then `Refunds::request()` per plan line and the announcement after it. The transaction was never the problem; the audience was.
- **Six questions, six answers, all recommendations accepted.** Staff-initiated cancellations alert every authorised colleague including the actor; the gate is `orders.view`, matching the page the alert links to; the message carries the money position in words with no figure; idempotency lives in the dispatcher; a failed send records a sanitised category; retry stays the manual resend and cron is unchanged. Recorded in the PRD Section 15 admin-alert line so the specification and the build agree.
- **What was built.** One new event `admin_order_cancelled` (staff audience) with seven tokens: `order_number`, `customer_name`, `cancellation_source`, `cancellation_reason`, `delivery_day`, `refund_state`, `admin_url`. `announceCancellation()` now reads the committed `order_cancellations` row with the actor's name joined on, so the source, the reason and the refund position are the ones the transaction wrote and a caller cannot invent them; no row means nothing is announced, which also makes a refused cancellation silent. The staff variables are built token by token rather than handed the customer context, for two reasons: `Mail::ctaFromVars()` takes the first address it recognises, so a context carrying `order_trail_url` would have put "Follow your order" on a message whose next step is the admin order, and a staff alert that never receives the customer's trail address cannot forward it by accident.
- **Recipients are read, never listed.** `Notifications::staffRecipients('orders.view')` at send time: active users only, exact permission or the `orders.*` wildcard, owner always qualifying, and the configured support inbox as the fallback when nobody holds it yet. No address, no user id, no recipient list, no order URL and no wording in `api/v1/orders.php`, which was not modified at all. The admin link comes from `APP_URL` and the order id through `orderContext()`.
- **Idempotency.** `Notifications::alreadyAnnounced($event, $relatedType, $relatedId)` asks the ledger before either half writes. Three layers now stand between a repeated request and a second email: the unique key on `order_cancellations.order_id`, the controller's `code === 'cancelled'` gate, and the dispatcher's own check. A send that failed the first time is deliberately not retried by the announcement: it is already a failed delivery row with a reason on it, and the "Send it again" button behind `notifications.resend` is the one place a retry is a decision rather than an accident.
- **Money position, no figure.** `staffRefundStateLine()` maps the ten statuses the cancellation engine can produce onto six plain sentences, and the unit suite asserts that none of them contains a naira symbol or a digit. The customer message keeps the exact figure, and the database suite asserts both halves of that split on the same order.
- **A failed send now explains itself.** `Mail::classifyFailure()` sorts a driver complaint into one of seven fixed sentences (unreachable, refused sign-in, timeout, address rejected, unusable sender, no transport, unknown) and returns only those, whatever it is given. `Mail::lastError()` holds the category for the send that just ran, and all three `recordAttempt()` calls in the dispatcher store it instead of the one literal they used to share. The raw PHPMailer text still goes to `error_log`, where it belongs; the category is what reaches `notification_deliveries.last_error`, which `admin/orders.php` prints on screen. The unit suite feeds it hostile strings carrying a hostname, a password and an internal relay name and asserts none of them survive.
- **Tests.** New `scripts/tests/CancellationNotificationTest.php` (unit, 378 assertions of its own: the catalogue, the bell boundary, all three helpers across every branch, the house law on the new copy, the classifier, the migration shape and token coverage, and the rule that the controller decides nothing). New `scripts/tests/cancellation_notification_db_test.php` for MySQL 8: authorised and unauthorised recipients, an inactive colleague excluded despite holding the role, in-app and email rows per channel, the seven facts in the rendered body, the dynamic admin URL, bell visibility and read state under `Rbac::loadFromDb()`, the customer half still working, staff cancellation naming the actor, the actor included, a retry and a repeated cancellation writing nothing new, refund outcomes, a refused cancellation alerting nobody, an announcement with no committed row writing nothing, and a mail server on port 1 that leaves the order cancelled, records a failed delivery with a sanitised reason, keeps the in-app copy, stays eligible for resend, and resends on the same row at attempt 2. Extended `AdminNotificationsTest.php` (bell permission), `cancellation_http_test.php` (the alert is produced by the real controller over HTTP, once, across a repeated request, with the manager fixture receiving it by permission) and `admin_notifications_http_test.php` (the feed and the history page return it with the Order 360 deep link, and it can be marked read).
- **Run here.** `php scripts/tests/run.php` 4,076 of 4,076, up from 3,683 on the same tree before the change; `php -l` clean on all 332 shipped PHP files; `bash scripts/brand-check.sh` all eight guards green; `npm install` and `npm run build` clean, with no JavaScript touched and no change to the committed stylesheet. Not run here, because this container has no MySQL server and no reachable package mirror for one: the two new and three extended database and HTTP suites, and `bash scripts/verify.sh`, which answers `[verify] no base URL` without a `.env` and a live site. Those run in the release gate, which starts the mail sink and the stand-in gateway. PHP itself had to be installed to run any of this: the container ships none, so a PHP 8.3.8 CLI was fetched into `~/.cache` and symlinked, and nothing about it is in the repository.
- **Discovered, not changed.** `refund_failed` is announced with the whole customer order context, so its staff email also gets a "Follow your order" button pointing at the customer trail rather than the admin order. Same wart, different event, and fixing it would alter a template that has shipped, so it is left for whoever owns the payments alerts. And `npm run build` rewrites `assets/js/admin-content.min.js` by one variable name against the committed bundle with no source change behind it, so the built file was reverted rather than carried into this diff.
...
- Fixed by pinning the real property, which is strictly stronger than the old check: exactly one POST form on the screen, addressed to the endpoint that owns the write rather than handled on the screen, carrying `Csrf::field()`, and rendered only where `Rbac::can('customers.create')` is true. The old check would also have passed for a screen with a self-posting form and no token. Same treatment this file already gave its placeholder-count and "one action" assertions when the code they described grew.
- Verified by execution, not inspection: the amended file runs 85 / 85 green against the real repo sources, and restoring the original assertion against the same `admin/customers.php` fails with the exact CI message. `scripts/brand-check.sh` 8/8. The sandbox has no PHP binary and no route to one (apt, packagist and the PHP CDN are all unreachable), so the file was executed through a PHP-in-Node runtime with `file_get_contents` bound to the real files; the whole unit suite still runs in CI.
- Integrated `origin/main` at `632d690` (PR #66) into this branch. The only conflict was two additive entries in this log, resolved newest-first with both preserved, matching the PR 67 resolution above. No application code conflicted: `main` moved on docs, evidence logs and the two test runners, none of which this branch touches.

### 21 Sep 2026, storefront and admin polish pass: eight Owner requests, six confirmations

Branch `arena/01a0c30e-okveggies`, from `main` at `77ba2b9`. The Owner sent six screenshots of okveggies.com.ng with eight requests. Per the build loop, six clarifying questions went out first, each with three options and a recommendation, and all six were answered before any code was written. What was built, in the order asked:

1. **The combo saving badge is red.** `includes/components/shop/combo_card.php`: the "You save ₦X" line moved to a single tomato badge on the top right corner of the combo photo. The struck-through component total stays once, under the price, as the price anchor, and the duplicate bottom badge is removed. Nothing else on the card moved (Owner: "Just move the you save to the RED badge not green. RED. Everything else remains same...").
2. **Make It Right sign in is red, page and sheet.** The signed-out "Sign in to report" button is now a solid tomato fill on the guidance panel and inside the help sheet. `includes/components/shop/help_sheet.php` gained a `danger` action style and an optional per-action icon, so any future sheet can carry the one button the reader has to see.
3. **The category buttons are buttons, on both surfaces.** The home chip strip under the hero and the "Shop by category" grid now use `okv-cat-chip` and `okv-cat-card` (new component classes in `assets/css/src/input.css`). Each button is forest-grounded with the primary photo of the first active product in that category behind a heavy wash: 10% opacity at rest, so the photo is texture, not subject. Hover lifts the wash to 35%, the arrow slides, and the card slides a "Shop {Category}" bar up from the bottom. `active:scale-[0.98]` answers a tap on touch screens, where there is no hover. `Catalogue::categories()` returns a `sample_image` per category in the same round trip (one correlated subquery on the indexed product images). An empty category keeps the plain forest ground, a faint leaf watermark and its "Being sourced" tag, so a missing photo never reads as a broken one.
4. **The How It Works cards are tappable.** Each of the three step cards is now a button that opens its own slide-up sheet on the existing `okv-sheet` pattern through `okv_help_sheet`. Step 1 carries "New here? Create an account" with a link to the register tab, and explains activation with the phone code. Step 2 covers the delivery-day sourcing rule, checkout, and the payment choices: card, bank transfer, USSD on a feature phone, and business credit for an approved Pro facility. Step 3 covers weighing, the weighed-produce photograph, the reporting window read from `IssueReports::reportingWindowDays()` rather than hard-coded, the photo cap, and the Make It Right link. The Learn sheet still holds the CMS body under `data-content-body`, so the published copy and the content tests keep their place.
5. **Our Story has a story and a photograph.** A full brand story is published for the about page in migration `057_story_content.sql`: the name, the farms visited in Ogun State and Jos, the price we can explain, the delivery day, the founder and CEO Kumbish Emmanuel Putleh and his why, and what we are not. It is restricted Markdown through the M12 renderer, so the Content module edits it like every other page. New static hero photograph `assets/img/story/story-hero-1280.jpg` (farm gate, crates of tomatoes, peppers, okra and onions, morning light). `page.php` no longer carries the "Approved documentary photograph pending" and "reserved for a rights-cleared photograph" placeholder: it falls back to the static image when no photograph is published, and to the seal if the file is missing. A photograph published through the Content module always wins over the fallback.
6. **Admin "Create customer", Owner only.** `customers.create` is registered in `includes/config/permissions.php` and in migration `056_customers_create_permission.sql`, granted to the Owner role and not to Manager, next to `users.create`. The API half already existed: `api/v1/customers.php` `create` had been gating on the unregistered key. `admin/customers.php` now renders a "New customer" panel (server gate plus `data-perm`) when no customer is selected: first name, last name, phone, optional email, household or business, business name. It posts to the existing `create` action, which validates through `StaffCustomers::validateNew()`, creates the account inactive, and redirects back with the new customer open and a green notice, or with the plain error from `StaffCustomers::message()`.
7. **The admin sidebar carries the person, admin only.** Per the Owner's correction, this is the admin panel, not the customer account page. The bottom of `includes/components/admin/sidebar.php` is now a pinned bar (the nav scrolls above it inside the aside): an initials avatar, the full name, the role badge, and "Your account" and "Sign out" as 44px icon buttons with aria labels. Sign out keeps its POST form and CSRF field; the icon is the target. New `user` and `logout` icons join the sidebar icon set.

Verification in this container: `npm run build:css` clean (it caught one real fault, `group` in an `@apply`, which Tailwind forbids; the class now rides in the markup). All 10 changed PHP files parse clean under a strict PHP 8.3 parser (php-parser stands in for `php -l`, which this container lacks). `scripts/brand-check.sh` is 8/8, including no em dash and no banned jargon in the new story and sheet copy. HTML tag balance was checked over every changed template and generated preview. The node before/after preview (outside the repo) now serves the home page with the red badge and the new category buttons, plus a How It Works page with working sheets at `how-before.html` and `how-after.html`. Migrations `056` and `057` are idempotent and end with their own verification queries.

Not verified here, because this container has no PHP and no MySQL: `php -l`, the unit, database and HTTP suites, and a live render of the admin screens. The deploy applies migrations `056` and `057`. Before merge: run the unit suite and one manual pass of `/admin/customers.php` as Owner (create a customer, watch it open with the notice) and of `/how-it-works.php` (each card opens its sheet, the window reads from settings).
...
- Current `main` already includes PR 65's correction to the document letterhead assertion, so the earlier hero CI failure is no longer an outstanding code difference. The existing PHP lint/unit, build and brand CI gates will run again on the integrated branch before the normal GitHub merge. No branch protection will be bypassed.
- Local checks on the integrated branch pass: full CSS/JS build with no generated-file drift, image contract **116/116**, lesser-text **128/128**, motion coverage **52/52**, brand guard **8/8**, and whitespace checks. Local PHP remains unavailable; CI provides the PHP lint/unit gate.
- The earlier request to keep PR 67 in draft is superseded by this merge instruction. PHP/MySQL-backed homepage and publication checks remain unverified locally, and wider PR6 and PR9 completion remains unchanged.

### 20 Sep 2026, full-screen homepage hero redesign

- **Scope and confirmations.** Read `CLAUDE.md`, the relevant PRD content/design/accessibility sections, this tracker, the remaining-fixes plan, audit photography notes, the ui-ux-pro-max reference notes, `index.php`, `ContentImages`, and the CMS draft/image/publication flow before editing. The working tree was clean. The five implementation answers were: the specified heading and introduction win on the homepage without changing stored CMS data; the hero gets its own minimum `100svh` below the header; its entrance replays when a visitor returns; CTAs retain natural wrapping; use available test tooling and open a draft PR rather than provision PHP/MySQL. No production database access or publication change was made.
- **Composition.** Replaced only the split hero with a semantic, full-background responsive image, a separate full-hero forest wash and unboxed foreground copy. The 120px seal, tagline, published eyebrow, CTA labels and destinations remain. The approved heading is `Bringing the Best of the Farm Straight to Your Kitchen.` and the introduction is `Freshness You Can Trust. Sourced daily from local farms, carefully selected, and delivered perfectly to you.` Forest is the wash, tomato is the primary CTA accent, and gold is the existing hairline and focus ring, never a button fill. The mobile wash is stronger towards the lower copy. Both CTAs are at least 56px high and can grow for the existing 60-character CMS limit. No navigation, lower-homepage, catalogue, checkout or admin redesign was made.
- **Image and CMS contract.** The chosen source `assets/img/brand/hero section.png` remains 1671 by 941. The committed candidates remain 640 by 360 (59,632 bytes), 960 by 541 (107,236 bytes) and 1280 by 721 (155,062 bytes), all byte-for-byte unchanged. The photograph is not represented as verified evidence of OK Veggies operations. The fallback alt stays `Crates of fresh tomatoes, red and yellow peppers, and onions.` Both preload and image now use the same `100vw` sizes, path and srcset; dimensions, object-cover, high priority, async decoding and eager loading remain. A valid published CMS image and its alt still win. `ContentImages::presentation()` now rejects missing, corrupt, wrong-format and mislabelled-width main files, and excludes invalid siblings from srcset. No homepage asset was added to generic upload handling, and the draft/publish flow is unchanged.
- **Motion.** Extended `initHero()` in the existing controller, with no new animation library or controller. The image scales 1.06 to 1 over 8 seconds, with a separate 7% scrubbed scroll drift and static CSS overscan so no image edge appears. The forest overlay stays still. Existing `back.out(1.2)` stamp, `power2.out` word stagger and follow-on CTAs, and `expo.out` scaleX hairline follow the documented presets. `gsap.matchMedia()` scopes the reduced-motion still state to this hero and cleans up on preference changes. Keyboard focus completes the entrance immediately rather than waiting for hidden CTAs. The no-JavaScript composition is entirely CSS, with no CSS entrance left on the hero. Other routes' motion policy is unchanged.
- **Verified locally.** Image contract **116/116**, lesser-text **126/126**, motion coverage **52/52**, existing motion browser regression **64/64**, brand guard **8/8**, JavaScript syntax, CSS build, targeted motion bundle build and `git diff --check` pass. Added PHP image and homepage guards plus HTTP scenarios for published overrides, invalid overrides, draft isolation, exact display copy and unchanged stored CMS copy. Those PHP scenarios are authored but not claimed as executed locally.
- **Browser fixture evidence, not PHP evidence.** `npm run test:hero` uses the actual hero markup and built assets with explicitly substituted fixture data when no `OKV_BASE` is set. It passes **165/165** assertions in Chromium 153 at 390 by 844 (DPR 2) and 1440 by 900 (DPR 1), with JavaScript on, off and reduced motion. Inspected the four requested JS/no-JS screenshots and the produce crop. No horizontal overflow, no console/page errors, complete image/overlay coverage, no glass/panel, 120px seal, touch-safe CTAs, visible gold focus and exactly **1** hero download in each state. Observed CLS is **0.000** in the JS and reduced-motion runs; CLS is not claimed as measured with JavaScript disabled. A conservative text-contrast test uses the brightest possible image pixel: the minimum white-copy contrast is **4.65:1** desktop and **4.90:1** mobile. Hero axe checks report **0** serious/critical violations. Return replay, preference toggling, maximum-length CMS labels, blocked motion libraries and parallax edges at 25%, 75% and 98% scroll also pass. Screenshots, JSON metrics and existing motion videos are outside Git at `/home/user/.cache/okv-hero-audit`. Run `OKV_BASE=http://your-local-php-host npm run test:hero` to repeat against a real page instead of the fixture.
- **Still blocked locally.** `php -l` was attempted for all 5 changed PHP files, and the PHP unit and homepage HTTP runners were attempted: PHP is not installed. MySQL, Apache and a local `.env` are also absent. `homepage_visual_test.mjs` cannot connect to the absent local PHP site; `bash scripts/verify.sh` reports no base URL; role smoke prerequisites are absent and no destructive smoke was attempted. Full PHP-rendered screenshots, database-backed publication/override verification and frozen-profile full-homepage performance remain pending. The existing GitHub CI is a separate PHP lint/unit gate, not a substitute for these integration checks. The draft PR must not be treated as deployment approval. **No wider PR6 or PR9 completion is claimed or marked.**
- **GitHub CI follow-up.** Draft PR [#67](https://github.com/praxisjbs/Okveggies/pull/67), implementation commit `8cfa7ee`. The PHP 8.3 CI [run 35521587198](https://github.com/praxisjbs/Okveggies/actions/runs/35521587198) passed lint for every shipped PHP file, the full CSS/JS build and the brand guard. The unit runner executed **3,455/3,456** assertions, with the new homepage/image assertions passing. Its sole failure is the existing `DocumentTest.php` assertion `the letterhead carries the single-ink mono-green lockup`. Verified the identical failure in the base commit `720625b` [run 35517060229](https://github.com/praxisjbs/Okveggies/actions/runs/35517060229), which reports **3,421/3,422**. Neither the document template nor its test changed in this task. The overall CI remains red, not waived; this unrelated failure and the PHP/MySQL integration gaps keep the PR in draft. No merge or deployment was performed.
...
- **Fix 3, Monday business day.** The 3 September decision is now data: `003_reference_seed.sql` seeds business Monday alongside Tuesday and Friday, and migration `051_monday_business_delivery_day.sql` activates it on databases seeded before the correction, inserting or activating the row without touching cutoff or lead times an admin already set. `delivery_db_test.php` now asserts the exact active business set (Mon, Tue, Fri) and that a business customer is offered a Monday. `Delivery::nextEligibleDates('business')` includes Monday through the existing rule engine, which needed no change.
- **Fix 14, cancellation asymmetry.** The Owner chose symmetry: after the cutoff an order paid in full forfeits the same share a deposit customer would, because the produce was bought either way. `Cancellation::depositShare()` computes the share (the recorded deposit, or the deposit percentage of the order total), `OrderCancellation::forfeitCap()` caps it at what was paid, and both the write path and the screen read path pass it into the unchanged `moneyOutcome()`. The checkout, FAQ, order screen and email copy now say "the deposit part of what you paid", and the Settings screen help states the rule plainly. `docs/CANCELLATION_ASYMMETRY_DECISION.md` records the question, the three options, the decision and the evidence. This closes the M13 contract Section 4 "Cancellation policy" dependency ahead of the release gate.
- **Fix 1, legal attest workflow.** Provable, page by page: `content_pages_db_test.php` now drives Terms, Privacy and Delivery Policy each through refuse-without-attestation, footer and sitemap exclusion while unpublished, attested publish, and an audit row naming the staff member who attested. `public_content_http_test.php` proves each of the three public paths returns the same branded noindex 404 while unpublished. A legal readiness panel on Page Copy lists the owed pages with a Waiting for client copy badge and links to each editor; it disappears when all three are published, so its presence is the honest state. `content_admin_http_test.php` pins its presence for content viewers and its absence from messages-only responses. The placeholders stay unpublished and out of footer and sitemap until the client supplies copy; that dependency is documented as P0 and unchanged.
- **Fix 18, M2 carry-forward.** The regex-over-SQL seed assertions are gone from `CatalogueTest.php`. The new `reference_seed_db_test.php` asserts the migrated database: 5 categories by identity, 4 units with the decimal rule, the 24 launch products, the Garlic kilogramme correction, image and availability rows, 30 zones, household days Mon/Wed/Thu/Sat and business days Mon/Tue/Fri. The empty-category question is decided: shown and labelled Being sourced, not hidden, pinned by creating a zero-product category and asserting `Catalogue::categories()` returns it with count 0. The suite is registered in `run_all.sh` and picked up by the release gate glob. The per-product source region part of the carry-forward stays open for PR4, as planned.
- Verification in this container: unit suite **3,380 / 3,380** (was 3,374; +8 cancellation assertions, +1 admin panel check, +3 automatic MySQL 8 compatibility checks on the new migration from `BootstrapTest.php`, and the 6 regex assertions removed), `php -l` clean on every touched file, `bash scripts/brand-check.sh` 8/8, `git diff --check` clean, no em dash, no banned jargon. The database and HTTP suites (`reference_seed_db_test`, `delivery_db_test`, `cancellation_db_test`, `content_pages_db_test`, `public_content_http_test`, `content_admin_http_test`) are written, linted and registered; this container has no MySQL 8 or Chromium, so they execute in CI and the release gate, as M12 recorded before.
- The admin readiness panel is the only UI touched. It reuses `okv-panel`, `okv-panel-head`, `okv-badge` and `min-h-[44px]` link rows, tokens only, no new CSS, so the compiled stylesheet is unchanged and `brand-check.sh` stays green. Browser pass at 390px and 1440px belongs to `content_admin_visual_test.mjs` in the gate.
### 20 Sep 2026, plan PR3: Checkout Beauty and Trust (fix 13)

- Built the checkout beauty flagship on `arena/01a0bd3e-okveggies`. Fix 13 first: the trust panel now sits below the whole payment radio group, as two small cards (shield for the Paystack payment, dotted road for the Order Trail) under a gold rule with the Paystack mark, so it can never read as a payment choice. That placement was the finding in `docs/M10_REVIEW.md` Section 4, and it is pinned by new assertions in `TrustSignalsTest.php` that fail if a payment radio ever lands below the panel again, plus the same order asserted over HTTP in `customer_http_test.php`.
- The four steps share a new progress line: 44px icon circles, a line between them, done steps ticked, the current step forest-filled and named with `aria-current`. The steps are a real ordered list.
- Payment choices are large tappable cards (`okv-choice`): a drawn 28px radio with a dot that springs in on Market Bounce, title, mono amount, and an illustration circle on the right (card, banknote, handshake, receipt). `fieldset` + `legend` keep the group accessible. The chosen card takes the forest border and gold ring through `has-[:checked]`, and the whole card is the tap target.
- The primary action is a sticky mobile pay bar (`okv-pay-bar`): total on the left, a 56px forest Pay now button with an arrow on the right, floating above the mobile tab bar with the safe area cleared. It submits the payment form by its `form` attribute, so it works with JavaScript off. `assets/js/checkout.js` moves its label with the choice (Pay now / Pay deposit / Place order) and turns it into a waiting state on submit, so nobody double pays by double tapping.
- Less text, more illustration: the fee explanation and the cancellation rule are now one line each beside an `i` mark, with the detail in bottom sheets (`fee-sheet`, `cancel-sheet`) that carry a bespoke 80px illustration. The cancellation sentence is still the one and only `Cancellation::policyLine()`, unchanged, so checkout, the order screen and the email keep saying the same thing. New `okv_delivery_policy_url()` (`includes/components/shop/policy_links.php`) links to the Delivery Policy when published, How It Works while it is not.
- The delivery day picker keeps the Lagos rules (`Delivery::nextEligibleDates`, untouched) and presents as a large day button that opens a 24px-radius bottom sheet with the eligible days, order-by evenings and a blurred backdrop. With JavaScript off it is the real select; one field name, one server contract. The shared `.okv-sheet` pattern gains the 24px radius and blur site-wide. New shared icon set in `includes/components/shop/icons.php` (single stroke, 24px grid, stroke 1.5). `cart.php` gets the same empty-basket illustration, gold hairline and arrow CTA.
- Motion is full, per the 20 Sep decision: the global `prefers-reduced-motion` collapse is gone from `input.css`, replaced by a 500ms entrance curve with per-card stagger (`okv-enter`), transform and opacity only. The three suites that pinned the old collapse (`AdminCommandPaletteTest.php`, `homepage_visual_test.mjs`, `visual_pass.mjs`) now assert the new full-motion contract. New tokens: `ink.10` for the resting card edge, `sheet` radius 24px. `tailwind.css` and the minified JS rebuilt and committed; `axe-core` added as a dev dependency for the new browser gate.
- Tests: unit suite **3,384 / 3,384** including the new trust-position assertions; brand gate **8 / 8**; `package.json` gains only `axe-core`. `customer_http_test.php` grew a full guest checkout journey over the real endpoints (add to basket, save steps, place, trail link opens, no account row, basket gone), with cleanup. New `scripts/tests/checkout_visual_test.mjs` drives 390/1440: step flow, trust placement in DOM order, card sizes, pay bar above the tab bar, 44px controls, no overflow, sheet open and close with focus return, arrow-key radio walk, axe-core with zero critical and serious allowed, and full motion with the reduced-motion preference set.
- This container has no MySQL and no Chromium (downloads blocked, apt offline), so `checkout_db_test`, the database-backed HTTP suites and the browser pass could not be executed here. Instead the page itself was rendered end to end on PHP 8.3 in WebAssembly against a stand-in `Database` outside the repo (the M12 method): all four steps plus `cart.php` render with no PHP warnings, the rendered DOM proves radio group, then trust panel, then policy line, then pay bar, every control labelled, no em dash, no `bg-gold` fill, no `focus:outline-none`, and the pay-on-delivery card correctly disabled for a guest. The database, HTTP and browser suites run in CI or on a MySQL 8 machine before merge.
- Post-open audit on a real Chromium (an npm-shipped headless build, since the browser CDNs are blocked) over the true rendered DOM and the built stylesheet, 95/95 assertions at 390px and 1440px: axe-core 0 critical and 0 serious on the payment step and the basket page, every control 44px and up, no horizontal overflow, the gold focus ring at `rgb(201, 146, 43)` on keyboard focus, the trust panel below the whole radio group in the live DOM, cards at 72px and up with the forest border and gold ring on selection, the pay bar 56px and full width floating directly above the tab bar, both sheets at a 24px radius with a blurred backdrop and focus returned, the day picker presenting as a button and writing the choice into the real select, and motion still full with the reduced-motion preference set. The audit caught four real defects, all fixed here: `cart.php` never required `icons.php` and so fataled on every view that reached the summary (P0, caught because the render stopped mid-page); the picker button carried Tailwind's `hidden` class, which `button.hidden = false` cannot clear, so the day sheet was unreachable with JavaScript on; the cart breadcrumb and line-title links sat under the 44px floor; and the footer copyright at `text-white/60` measured below 4.5:1 on forest, which the new axe gate on checkout rightly refused. `checkout_visual_test.mjs` now drives the cancellation sheet on step 4 (the fee sheet trigger lives on step 3), waits out the 240ms Botanical transition before reading the ring, and freezes animations for axe's measurement only.
...
- Fixed 2 real image-workflow defects: the controller now calls the existing transactional `ContentPages::updateDraftImage()` method, and responsive-image cleanup now captures the stored natural width before deleting the complete generated set. Corrected the editor guidance to say HTML is rejected, and added a cleanup regression test. The local PHP route fixture now faithfully models Apache trailing-slash and sitemap rewrites.
- The publishing contract remains fail-closed. Public reads return only published snapshots; unknown and unpublished pages return the same noindex 404; previews require `content.view` and send noindex; all writes require POST, CSRF and `content.edit`; restricted Markdown rejects executable HTML; legal publication requires explicit client approval; optimistic fingerprints reject stale writes; and content audit events are appended transactionally.
- Permission evidence covers the seeded Owner and Manager plus content-only, messages-only, content-view-only, content-editor and no-content custom roles. Content-only HTML contains no customer-message probe; messages-only HTML and JSON contain no draft probe; refused script markup is absent from validation JSON and staff preview. The unchanged Messages regression passed 27/27 database and 42/42 HTTP assertions.
- Verification on MySQL 8.0.46: all migrations applied from empty and the second run had nothing pending; ContentPages passed 51/51 database assertions; Content admin passed 53/53 HTTP/RBAC assertions; public pages passed 52/52; sitemap passed 18/18; homepage passed 20/20; and the full unit suite passed 3,326/3,326. The focused guest, household and business storefront suite passed 8/8, while the wider customer HTTP regression passed 132/132. Browser checks passed 29/29 homepage, 42/42 admin and 40/40 public-content assertions at 390px and 1440px, including keyboard disclosure operation, JavaScript-disabled FAQ answers, reduced motion, 44px controls and no overflow. One cold throttled homepage run measured 3,780ms and passed 28/29; its immediate repeat passed 29/29, so the result is recorded rather than hidden.
- Static and deployment evidence: 45 touched PHP files passed `php -l`; 9 touched JavaScript files passed `node --check`; touched shell scripts passed `bash -n`; all 8 brand checks and `git diff --check` passed. A disposable real-Apache deployment passed 29/29 verifier checks, including canonical redirects and 403 denial of `.env`, `includes/`, `migrations/` and `docs/`.
- The repository-wide runner reported 48 suites passed, 6 failed and 2 skipped in this isolated environment. Every M12 and Messages suite passed. The failures were outside M12: a pre-seeded visual credit fixture affected the M11 zero-state test; SMTP was unavailable for notification/settings mail assertions; the host PHP lacked `XMLWriter` for the pricing spreadsheet; and 2 suites could not create temporary triggers because the Docker MySQL account lacks `SUPER` or `log_bin_trust_function_creators`. These results do not close either M12 headline criterion.
- Dependencies remain explicit. No client-approved Track 2 real-operation photograph is present, so the homepage shows the approved dependency state and the first checkbox stays open. Terms, Privacy and Delivery Policy still contain unapproved placeholder drafts and remain unpublished, absent from footer and sitemap; the second checkbox is partial. No placeholder is approved as legal advice. No commit or push was made.

### 16 Sep 2026, M12 routing, navigation and discoverability

- Kept the fixed Task A routes and made `ContentPages` the one managed-page source for labels, canonical paths and legal grouping. Footer configuration now selects only the approved slug order, while Contact and shop destinations remain independent. A footer lookup failure stays fail-closed for managed content without removing those independent routes.
- Added slashless Apache normalisation and same-origin permanent redirects from recognised legacy `page.php?slug=...` URLs. Extra legacy parameters are discarded. Unknown, missing and unpublished requests retain one branded 404 and now send both HTML and HTTP noindex directives. Staff preview also sends an HTTP noindex directive and still requires `content.view` on every request.
- Added a fail-closed `/sitemap.xml` backed by published `ContentPages` state and dedicated Catalogue sitemap reads. It contains the homepage, Shop, Combos, Kitchen Runs, Contact, active products, currently buyable combos and published managed pages. It excludes category query variants, drafts, previews, unpublished content and legacy content URLs. Reliable publication or record dates become `lastmod`; database failure returns a complete XML-safe 503 with `Retry-After` and noindex. Static `robots.txt` advertises the production sitemap.
- Verification passed: 3,320/3,320 unit assertions, 52/52 public content HTTP assertions, 18/18 sitemap HTTP assertions, 51/51 ContentPages MySQL 8 assertions, 38/38 Content admin HTTP/RBAC assertions, 42/42 Messages HTTP assertions and 20/20 homepage HTTP assertions. PHP and shell syntax, all 8 brand checks, `git diff --check` and the full real-Apache deployment verifier are green. The verifier proved clean redirects plus 403 denial of `.env`, `includes/`, `migrations/` and `docs/` locally. No migration was required, and no commit or push was made.

### 16 Sep 2026, M12 Task F homepage acceptance pass

- Kept the existing Catalogue-backed categories, product cards, combo cards, basket notices and add-to-basket forms. The homepage now reads only the published `home` snapshot for editable marketing copy and SEO, falling back to reviewed public defaults when that snapshot is absent or unpublished. Draft homepage text never reaches public HTML.
- Added the dedicated OK Veggies promise and clear Shop, Combos and Kitchen Runs routes. Featured combos now mean explicitly featured and currently live. Combo, category and product sections keep their headings and provide honest empty or isolated database-error routes instead of disappearing. Zero-product active categories remain filter links labelled "Being sourced".
- Replaced the featured-combo hero card with the final responsive documentary-image region. Content staff can upload a rights-cleared real-operation image for Homepage or Our Story; the controller rechecks POST, CSRF and `content.edit`, requires meaningful alt text, protects against stale edits, and generates randomised responsive WebP variants under `uploads/content`. The hero alone is preloaded and high priority; card images remain lazy-loaded.
- Added homepage-specific title, description, canonical URL and Open Graph URL/image handling. The responsive pass caught and fixed a 36px mobile logo link and removed the non-critical JetBrains Mono preload that pushed throttled first paint past the PRD budget.
- Verification passed: 3,292/3,292 unit assertions, 51/51 ContentPages MySQL 8 assertions, 20/20 homepage HTTP assertions, 37/37 Content admin HTTP/RBAC assertions and 29/29 homepage browser assertions at 390px and 1440px. The browser pass includes 4x CPU throttling, a 1.6Mbps/150ms mobile network profile, first contentful paint under 3 seconds, initial resources under 2MB, reduced motion, 44px targets, no overflow and lazy below-hero images. PHP and JavaScript syntax, the brand gate, the full Apache deployment smoke and `git diff --check` pass.
- External dependency: the repository contains product photographs and brand marks, but no client-approved Track 2 documentary photograph. No stock or synthetic substitute was added. The first M12 checkbox remains open until the client supplies and staff publish a rights-cleared real-operation photograph.
...
- Task A fixed the complete content contract in `docs/M12_CONTENT_CONTRACT.md`: 7 stored slugs and clean canonical paths, restricted Markdown, heading-based FAQ data, separate draft and published snapshots, session-bound preview, SEO fields, fixed homepage copy fields, rights-cleared real-operation photography, legal-copy attestation, footer-only links, separate Content and Messages permissions, audit history and fail-closed public states.
- Task B adds the shared `ContentPages` service and registers it through bootstrap. The service returns presentation-neutral arrays, never HTML. Public reads require both a supported slug and `is_published = 1`; admin and preview reads remain separate for route-level `content.view`. Every mutation validates an active staff actor, locks the fixed row, compares a SHA-256 draft fingerprint where applicable, uses prepared values, writes the content and its append-only audit event in one transaction, and lets unexpected database exceptions reach the controller for safe logging.
- Migration `049_m12_content_pages.sql` is the one schema change the approved staged-publication contract needs. It adds exactly the 14 approved draft, SEO, homepage, image and publication-provenance columns, plus the `published_by` foreign key. It backfills working drafts, seeds the fixed homepage, and unpublishes legal rows only while their published body is still the original placeholder, so a forced rerun cannot withdraw later approved copy. Migrations `001` and `006` were not changed.
- Focused pure tests cover the registry, canonical paths, restricted formatting, unsafe HTML and links, length limits, incomplete drafts, publication validation, FAQ parsing and limits, homepage field and CTA allowlists, stable fingerprints, migration guards and bootstrap registration. The full unit runner passes 3,143/3,143 assertions. PHP lint, shell syntax, `git diff --check` and the brand gate pass.
- `scripts/tests/content_pages_db_test.php` is registered in the normal database gate and restores every page and test actor it touches. It covers public and preview isolation, draft-only writes, unrelated-row isolation, stale conflicts, active actor checks, image separation, legal attestation, publishing, unpublishing and page-scoped audit history. On MySQL 8.0.46, migration `049` applied in 1,479ms, a second migration run reported nothing to apply, and the focused suite passed 43/43. Its first run exposed one incorrect test expectation that used literal backslashes instead of real line breaks; correcting the assertion left the service unchanged and the rerun passed fully. The production deployment smoke passed 23/25 checks, but `/migrations/` and `/docs/` both returned HTTP 200, so the long-standing protected-directory release blocker remains open. No controller, admin page or public presentation task was implemented, and no M12 headline criterion was closed.
- The protected-directory failure came from the deployment seam: the staged tree contained `.htaccess` and `.user.ini`, but the SFTP action uploaded `./_dist/*`, which does not reliably carry dotfiles. Production confirmed the broader exposure by serving `README.md`, `composer.json`, `scripts/verify.sh` and `docs/PRD.md`. The workflow now uploads both root control files explicitly, `.htaccess` has an early rewrite-level directory denial as well as its existing file and alias rules, and the workflow runs the full smoke suite after migrations. `DeploymentSecurityTest.php` locks those controls in place. The fix is not live until this branch is deployed.
...
- Merged current `main` (M10) into the branch and resolved a cross-milestone collision. The branch was cut before M10 landed, so it reinvented M10's `admin_new_issue_report` alert with different token names (`issue_category`/`message_preview`), a re-defining migration `042` using `ON DUPLICATE KEY UPDATE`, and an uncalled `announceIssueReport()`. On a live M10 database that migration would have overwritten the working template with mismatched tokens (blank category and preview on every staff alert) and wiped Owner edits. Removed the duplicate `EVENTS`/`TOKENS` keys and the dead sender, and reduced `042` to the genuinely new `admin_manual_payment_proof` template with `INSERT IGNORE`. M10's alert stands untouched.
- Rebuilt `assets/css/tailwind.css` and the JS bundles from the merged source.
- Verification: 3,066/3,066 unit assertions passed, `php -l` clean on every touched file, all 8 brand checks green, `git diff --check` clean. The MySQL and HTTP suites need a database and were green in the delivery runs; they run in CI against the merged base.
...
- Expanded `scripts/verify.sh` to cover every M10 entry point: the write endpoint refuses GET, the private customer order view fails closed without revealing whether an order exists, the staff queue requires sign-in, and a missing private photo returns 404. The existing bad-token trail check remains the public privacy probe.
- Proved the migration range on a fresh MySQL 8 database. Migrations `045`, `046`, `047` and `048` applied through the normal runner; the immediate second run reported nothing pending, and no migration uses reserved slot `049`.
- Verification: `bash scripts/tests/run_all.sh` passed 46 suites with 0 failures against the fresh database, local SMTP sink and stand-in Paystack gateway. The runner reported 1 environment skip for its Playwright script because that package is not installed; the M10 screen had already passed the required browser checks at 390px and 1440px in Task H. The expanded local deployment verifier passed 23 application, route, asset and access checks. Its 2 Apache-only directory-denial checks correctly remain red under PHP's built-in server, which does not apply `.htaccess`; both must return 403 or 404 on Apache or cPanel before go-live. No commit or other Git history change was made.

### 10 Sep 2026, M10 Task H: complete issue lifecycle verification

- Expanded the database workflow suite from its happy path into the full state graph. An open report cannot jump to a terminal result, an in-progress report cannot use an obsolete expected state, another colleague cannot finish it, and resolved or declined reports cannot be taken, reassigned or resolved again. Every refused move leaves the permanent history unchanged.
- Strengthened the real M5 refund proof. A value above the selected immutable order line creates no refund; the accepted decision stores exactly ₦2,700, links the actual Paystack transaction and writes the same amount on the issue outcome. Expanded the public Order Trail assertions so report descriptions, states and internal issue names remain private.
- Put the seeded Manager through the normal take and decline path and the seeded Owner through the same report workspace, alongside guest, owning-customer, other-customer and partial custom-role checks. Added every M10 database and HTTP suite to `scripts/tests/run_all.sh`, so the normal repository gate can no longer omit resolution, outcome or notification coverage. Updated the Settings regression to include the M10 reporting-window field that its whole-tab count had missed.
- Verification: `bash scripts/tests/run_all.sh` passed 46 suites with 0 failures against a fresh MySQL 8 database, the local SMTP sink and the stand-in Paystack gateway. This includes 2,970 unit assertions; 102 M10 database assertions across reporting, workflow, resolutions, customer outcomes and notifications; and 142 M10 HTTP assertions across reporting, hostile photos, roles, CSRF, methods and resolutions. The runner skipped its Playwright script because that package is not installed, so the Make It Right screen was checked separately in the app browser at 390px and 1440px with no page overflow and no visible main control below the applicable 44px mobile or 40px desktop target. PHP and shell lint, the production asset build, all 8 brand checks and `git diff --check` passed. The local route verifier passed 21 application checks; its 2 Apache-only directory-denial checks return 200 under PHP's built-in server because it does not read `.htaccess`. No commit or other Git history change was made.

### 10 Sep 2026, M10 Task G: issue notifications

- Completed the M6 notification seam for Make It Right. `issue_report_received` now supplies the report category and a safe description preview as customer-template tokens. The new-report staff alert is addressed to active users through the `issues.view` permission rather than a hard-coded role list, with the configured support address retained only as a fallback.
- Kept the domain record authoritative: report and resolution writes commit before the dispatcher runs, delivery failures remain in `notification_deliveries` for the existing retry worker, and no issue path calls `Mail::send()` directly. The resolved message carries the exact customer-facing outcome reason for refunds, credits, replacements and declines; a refund also starts the existing M5 refund notification sequence.
- Added migration `048_make_it_right_notification_copy.sql` to enrich the default receipt copy without overwriting wording an Owner has already edited. The three report templates remain editable, token-validated, previewable and test-sendable through the existing Notifications settings tab.
- Verification: the full unit runner passed 2,970/2,970 assertions. The focused Task G MySQL 8 suite passed 10/10, the notification database regression passed 144/144 against the local SMTP sink, the issue-report database suite passed 24/24, and the issue workflow HTTP suite passed 38/38. Migration `048` applied cleanly and a second migration run had nothing to apply. Browser checks at 390px and 1440px found all three templates and the new tokens with no horizontal overflow and no visible notification control below 44px. No commit or other Git history change was made.
...
- Added `IssueResolutions`, the shared terminal-outcome service. The handler selects the affected immutable order lines, and their stored totals cap a refund or account credit. Exactly 1 outcome is recorded. It uses row locks, expected-state checks, permanent history and audit rows; terminal replays cannot create another money or fulfilment action. The Owner has an explicit audited takeover path instead of silently bypassing `handled_by`.
- Refund delegates to M5 `Refunds::request()` and needs `payments.refund` as well as the issue permissions. A nullable unique issue link on the refund row prevents concurrent duplicate gateway calls and lets a retry recover the existing result. The report resolves once the refund is raised, while the customer wording continues to come from `Refunds::customerStatusLine()` through pending, processing, processed or failed. A failed result remains on the record for staff follow-up and is never retried automatically.
- Account credit delegates to the M8 credit journal and additionally needs `credit.grant`. It is available only to a business with an approved facility, is an append-only signed adjustment with an idempotent issue source key, and remains visible but disabled with an honest reason for a household or an unauthorised colleague. Replacement requires an active, staff-created order for the same customer and records `replacement_order_id`; it never rewrites the original order.
- Migration `047_make_it_right_resolutions.sql` adds the outcome amount, credit and replacement links, the unique refund-to-issue link, foreign keys and `issue_report_resolution_items`. It applied successfully in the complete chain on MySQL 8.0.46, and migration status reports every migration through `047` as OK.
- Verification: the full unit runner passed 2,916/2,916 assertions; the focused Task D database suite passed 16/16 against MySQL 8 and the local fake Paystack service; the Task D HTTP suite passed 12/12; Task C database and HTTP regressions passed 32/32 and 38/38; M8 credit-order regression passed 48/48; the M5 refund/cancellation gateway regression passed 24/24. All touched PHP files passed `php -l`, `scripts/brand-check.sh` passed all 8 checks, the CSS and JavaScript builds completed, and `git diff --check` was clean. Authenticated Owner browser checks at 390px and 1440px found no page overflow, no undersized visible main control, the full resolution form, and the manual-order replacement link. Local `scripts/verify.sh` passed 19 route and asset checks; its 2 expected Apache-only directory-denial checks failed under PHP's built-in server because it does not read `.htaccess`. No commit or other Git history change was made.

### 9 Sep 2026, M10 Task C: staff queue, ownership and decline

- Replaced the Make It Right placeholder with an oldest-first active queue, status, category and received-date filters, 25-row pagination, and a responsive list/detail workspace. The detail keeps the customer report, private photo thumbnails, order, customer, immutable item snapshot, delivery context and M5-maintained paid amount together. The report and admin order link to one another.
- Extended the shared `IssueReports` service with staff reads, labels, locked take and decline transitions, expected-state checks, same-handler take idempotency and permanent append-only history. `open` moves to `in_progress` when an authorised colleague takes it; `declined` is terminal, requires a 10 to 1,000 character customer-facing reason, clears the active slot and preserves the report. Both transitions are audited. The controller requires `issues.view` and `issues.resolve`, POST and CSRF for every workflow write. A committed decline queues its customer notification afterwards.
- Added migration `046_make_it_right_workflow.sql` for `handled_at`, the longer outcome note and `issue_report_history`, including idempotent backfill. It applied in the full migration chain on MySQL 8.0 and reapplied directly with one `handled_at` column, one history table and zero duplicate reported-history rows.
- Verification: `php scripts/tests/run.php` passed 2,896/2,896 assertions; `issue_workflow_db_test.php` passed 32/32; `issue_workflow_http_test.php` passed 38/38; the earlier issue suites passed 24/24 database, 41/41 report HTTP and 39/39 photo HTTP; `order_lifecycle_db_test.php` passed 11/11; `notifications_db_test.php` passed 144/144 through the local authenticated Mailpit test double; all touched PHP files passed `php -l`; `bash scripts/brand-check.sh` passed all 8 checks; `npm run build` passed; `git diff --check` was clean. At 390px the page has no horizontal overflow, every visible action is at least 44px high and keyboard focus has a 2px visible outline. At 1440px the queue and detail render as the selected two-column workspace with no horizontal overflow. The browser first caught that the new grid rule was absent from the built CSS; rebuilding fixed it. The first metadata assertion assumed lowercase information-schema keys and was made driver-tolerant. The first notification regression lacked its SMTP double; rerunning through Mailpit passed.
- Refund, account credit and replacement controls are visible but disabled with a clear explanation. Task D must connect them to the existing Refunds, credit and order services before the combined admin-resolution checklist line can be ticked. The full customer and role browser matrix remains for Tasks E and I.
...
### 20 September 2026, shop outage: production database is missing `products.source_region`
- Live `shop.php` and `product.php` returned a blank 500 while the home page, Combos and the combo detail all answered 200, and the home page's "This week's picks" section showed its degraded fallback. Every failing read selects `p.source_region` (added by migration `053_product_source_region.sql`); every working page avoids it. The production database is behind the deployed code: migration 053 (and 054) has not been applied there.
- Restoring the shop means applying the pending migrations on the server, then reloading the shop: `curl -H "X-Migrate-Token: YOUR_TOKEN" https://okveggies.com.ng/public/migrate.php` must end in `MIGRATE OK`, and `.../public/migrate.php?action=status` must read `OK` for 053 and 054. No code change can substitute for the missing column.
- Hardened the catalogue reads on branch `arena/01a0bf66-okveggies` so a database failure degrades instead of blanking: `shop.php` catches a catalogue failure and answers 503 with `Retry-After: 300` and a friendly state pointing at Combos and Kitchen Runs; `product.php` does the same for the product read and degrades photos and suggestions independently; `api/v1/catalog.php` wraps every action so a failure is a JSON 500 the live search can handle.
- Closed the process gap that let this ship silently: `scripts/verify.sh` now smoke-checks `/shop.php`, `/combos.php`, a product page and the `browse` endpoint, pinned by new assertions in `scripts/tests/DeploymentSecurityTest.php`, and `docs/DEPLOYMENT.md` names the shop check in the verify steps.
- Not run here: this container has no PHP and no MySQL, so `php -l`, the unit suite and `verify.sh` against production still need a green run in CI and on the release box before merge.

### 20 September 2026, deploys unblocked: migration 003 restored to its applied bytes
- Symptom: every deploy to production from the PR #57 merge onward (deploy runs #50 to #57, covering PR #57, PR #60, PR #61, the hero image pushes, PR #63 and PR #64) failed at the "Apply migrations on the server" step. The SFTP uploads all succeeded, so production kept receiving fresh code while the database stood still: migrations 051, 053 and 054 never ran on the server. That is the outage the entry above describes: `shop.php` and `product.php` blanked on the missing `products.source_region` column, and only PR #64's degraded catalogue fallback kept the pages answering.
- Root cause: PR #57 edited `migrations/003_reference_seed.sql` so the seed itself would carry business Monday. The intent was right, but 003 had already been applied to production on 26 August. The Migrator records a SHA-256 checksum for every applied migration in `schema_migrations` and, before applying anything, checks every applied file against its recorded checksum. The edited 003 drifted, so the runner threw "Refusing to skip 003_reference_seed: checksum changed. Create a new migration or force." on every run. `public/migrate.php` answered 500 with `MIGRATE FAILED`, the deploy step's `grep -q "MIGRATE OK"` failed, and every later deploy failed the same way because the drift never left. Migration 051, which carries the Monday decision correctly, never got its turn.
- Fix: restored `migrations/003_reference_seed.sql` byte for byte to the version production applied. Verified with git blob hash `579432e65c3783c0b583c8d89e18cc397c1f9767`, identical to the file at the last green deploy (`f527ff4`, PR #56). Nothing is lost: migration 051 inserts or activates the Monday business row on every database, fresh installs included, and leaves any admin-set cutoff or lead time untouched. Updated the 051 header and the delivery test comments that had started claiming 003 seeds the Monday row, and stated the append-only rule in the 051 header so the next person sees it before editing a seed.
- Expected effect on the next deploy: the runner sees 003 as OK, applies the pending 051, 053 and 054 in order, answers `MIGRATE OK`, and the verify step smoke-checks the shop, a product page and the browse endpoint again. No manual server step is needed; the workflow's own migration call does it.
- House rule, now enforced by experience: migrations are append-only. A change to data that an applied migration touched belongs in a new migration, always. The checksum guard in `includes/classes/Migrator.php` is what enforces this, and this incident is the proof that it bites. If an applied migration is ever genuinely wrong on the server, fix it forward with a new migration, or use the runner's force path deliberately, never by editing the file under a live database.
- Second breakage found and settled while diagnosing the deploys: the unit suite has been red since the "logo harmony" push (commit `1467fe7`, 20 September 2026), which moved the document letterhead from the single-ink `lockup-mono-green.svg` to the full-colour `lockup.svg`. `DocumentTest.php` enforced the old single-ink reading of bible 3.7a, so every CI run since that push failed at the unit step. The Owner confirmed the full-colour lockup is the intended look, so the test now pins `lockup.svg` on the letterhead and records the decision, and the brand gate's asset-existence rule is untouched (the single-ink set stays in the brand assets for the uses that still need one-colour reproduction).

### 20 September 2026, PR2 Fix 30: "Pick from shop" is a picker, and a shop list quotes itself
- The Owner's report: the Kitchen Runs page offered a "Pick from shop" start, then step 2 showed the same free-text rows as every other mode, with no product picker and no shop prices. The backend was already ready (`kitchen_run_items.product_id`, server-side hydration of name, unit and price, `price_source = 'catalogue'`), and an older row component even had a product dropdown, but the 3-tap rewrite had stopped using it. Decisions taken with the Owner before building: a dropdown plus quantity per line; a separate free-text block for off-shop items; live line totals and a running total; keep the four mode cards and fix step 2; and a shop-picked list is auto-quoted at shop prices, so the customer only approves.
- `kitchen-runs.php` step 2 is now two sections. The shop section renders picker lines from the new `includes/components/shop/kitchen_run_shop_row.php` (a dropdown of active priced products labelled "Name, ₦X per unit", a quantity, and a line total that computes itself), "Add a line" under the rows, a running-total card, and a collapsible "Anything not in the shop?" block. The typed section holds the free-text lines for the other modes, rendered by the rewritten `kitchen_run_row.php` (which the 3-tap rewrite had orphaned; its catalogue dropdown is gone because the shop section owns that job now). Only the section the chosen mode needs is enabled: the hidden section's fields are disabled, so a line typed under one mode can never ride along unseen under another. Step 3 repeats the shop total above Send. Saved Pro lists prefill the right rows: shop lines into pickers, typed lines into the block, which opens itself when there is something typed in it. The two dead help sheets from the rewrite are removed, the dev-speak "44px fields" subtitle is gone, and the empty state no longer promises "we will price your list" to a customer whose list just priced itself.
- `kitchen-runs.js` rewritten around the two sections: mode cards swap sections, subtitles and help copy; rows add, remove and renumber across one shared `items[]` index (shop lines first, then typed); totals compute from the option's `data-price` (display only; the server re-reads prices from the products table) through `OKV.money`, with a local fallback if the module is absent; validation on Continue and on Send names what is missing in plain sentences; a successful fetch send follows the redirect URL the API now includes in the JSON body, so the quoted and submitted flags on the landing URL are the server's decision, not the browser's guess.
- Auto-quote: new pure rule `KitchenRuns::autoQuoteTotal($mode, $pricing, $open, $lines)` returns the kobo total when a customer's own 'catalogue' + 'by_us' list is complete shop lines, and null otherwise (a typed line, an open budget, an empty list, a line without its price source or total, or any other mode or pricing). `KitchenRunWorkflow::submit()` applies it inside the transaction, stores the total on `quoted_total_subunit` and `estimated_total_subunit`, stamps `quoted_at`, and moves the request through the ordinary `transition()` gate to Quoted with a trail note the customer can read. Conversion still re-sums the stored lines and proves they equal the approved figure, unchanged.
- Notifications: new event `admin_kitchen_run_shop_priced` (EVENTS, TOKENS, `announceKitchenRunShopPriced()`, migration `055_kitchen_run_shop_priced.sql` with its template) so the team's email says "check the list and convert it", not the "fill in the prices" that `admin_new_kitchen_run` would say. The controller sends the shop-priced alert plus `kitchen_run_quoted` for a self-quoted list, and the ordinary pair otherwise; the landing URL carries `quoted=1` and the page answers "Shop prices attached. Check the total and approve."
- PRD Section 8.3 records the decision. Tests added: 9 unit assertions for `autoQuoteTotal` (exact kobo sum, and every refusal), 3 more registration and wiring assertions across the notifications and kitchen-runs suites, 13 database assertions in `kitchen_runs_db_test.php` (a shop list quoted at send-off with its trail, approved the ordinary way; an open-budget shop list and a staff-typed shop list both staying Submitted), and an HTTP case in `kitchen_runs_http_test.php` (submit over the real route with a product line, expect `&quoted=1` in the redirect and the total in the answer).
- Verified in this container as far as it allows: the container has no PHP binary, so the unit suite ran under a WebAssembly PHP 8.3 with the host filesystem mounted, green at 3,455 assertions; all 322 shipped PHP files parse clean under a strict PHP 8.3 parser; the page JavaScript passed 30 behaviour checks in a DOM harness that mirrors the rendered markup (mode switching, add and remove, live totals, validation, disabled hidden sections, the posted field set), which caught two real defects before commit: an undeclared `addShopBtn` and a unit `<select>` missing its `data-kr-unit` hook; `bash scripts/brand-check.sh` is green; `assets/js/kitchen-runs.min.js` and `assets/css/tailwind.css` are rebuilt. Not run here: the database and HTTP suites and a browser pass at 390px and 1440px, which need the MySQL 8 release box.

### 21 September 2026, Our Story: the founder portrait in "The person behind it"
- The Owner asked for the CEO's photograph on the Our Story page, only inside the "The person behind it" section. Placement was left to us with the condition that it works on mobile: the portrait sits under the section heading and above the paragraph, so every viewport reads heading, face, name, story. The header photograph slot is untouched.
- `includes/functions/helpers.php`: new `okv_story_founder_portrait($html, $portrait)`. The page body is admin editable Markdown, so the helper locates the deterministic heading anchor (`the-person-behind-it`) that ContentRenderer emits and inserts the figure directly after the closing tag. The width and height attributes are measured from the committed photograph at render time with `getimagesize`, so the markup and the file can never disagree. If the heading is renamed or the photograph is not in the tree, the body renders exactly as written: the page never breaks because of the portrait.
- `page.php`: the Our Story branch passes the portrait config (anchor, file, cache busted URL through `okv_asset`, alt text, caption). The caption reads "Kumbish Emmanuel Putleh, Founder". A comma, not an em dash, because the house law forbids the em dash in anything that ships.
- Tests: new `scripts/tests/FounderPortraitTest.php` under the unit runner: the figure lands after the heading, alt and caption are present, explicit width and height, lazy and async attributes, exactly one figure added, and all three no-op paths (no heading, no photograph, unknown anchor). `bash scripts/brand-check.sh` green; `node scripts/tests/image_contract_test.mjs` green at 116/116.
- The photograph arrived on `main` as `assets/img/brand/kumbish ceo.jpeg` (1254 by 1254, quality 90, 161kB, a space in the name). Moved to the story folder as `assets/img/story/founder-kumbish-emmanuel-putleh.jpg` (1080 by 1080, quality 82, metadata stripped, 109kB), the original removed, and `page.php` pointed at the new path. The page now shows the portrait in the "The person behind it" section as soon as this branch merges.
- Not run here: the container has no PHP binary, so the unit suite still needs its green CI run as usual.

### 23 September 2026, Admin catalogue settings: category and unit management modal
- Added Settings button with cog icon to `admin/products.php` beside the Add a product button in header actions, gated on `products.edit`.
- Built tabbed modal `includes/components/admin/catalogue_settings_modal.php` for managing categories and units of measurement, matching desktop dialog and mobile sheet patterns.
- Slugs for existing categories are kept immutable (Option C) to protect active URLs, while newly created categories auto-generate unique slugs.
- Unit editing allows modifying all fields including `allows_decimal` with safety checks requiring explicit confirmation if products are actively linked.
- Encapsulated CRUD and validation in `CatalogueSettings`, wired endpoints in `api/v1/products.php` (`category_list`, `category_save`, `unit_list`, `unit_save`), and recorded changes to `audit_logs`.
- Frontend module in `assets/js/admin-products.js` handles tab switching, live asynchronous list loading, and in-place add and edit operations. Rebuilt JS and CSS.
- Unit suite green at 3,556 assertions including new `CatalogueSettingsTest.php`. All brand checks pass.

### 23 September 2026, the empty state printed its own attributes as copy
- The Owner's report, from the basket page: the page showed `< id="okv-empty-6d2ad963" class="mt-5 font-editorial text-okv-h5 text-ink">Your basket is empty</>` as text, with the attributes on screen and the heading underneath. The block still had its illustration and its buttons, so only the heading was broken.
- Root cause, one line, `includes/components/shop/empty_state.php`. The component read its `heading_tag` option twice: `in_array(($opts['heading_tag'] ?? 'h2'), ['h1','h2','h3','p'], true) ? (string) $opts['heading_tag'] : 'h2'`. A caller that passed no `heading_tag` took the true branch on the strength of the `?? 'h2'` default, and the second read hit a key that did not exist, which is null, so `$tag` came out as an empty string. The template then printed `<` and `>` around nothing, PHP raised `Warning: Undefined array key "heading_tag"`, and HTML parsed `< ` as text, which is why the attributes landed on the page as copy. Reproduced verbatim under PHP 8.5 before touching anything.
- Fix: read the option once into `$headingTag`, trim it, whitelist that variable, and fall back to `h2`. Also null-guarded the action `href` and `label` reads, which would warn the same way on a badly shaped action. No call site changed, because the component owns the default.
- Every surface that showed it: `cart.php:81` (the basket, the reported one), `combos.php:126` (no combos this week), `includes/components/shop/shop_results.php:75` and `:85` (a search with no match, an empty aisle), `kitchen-runs.php:402` (no runs yet) and `page.php:222` (an FAQ page with no published questions). Six of the twenty call sites. `kitchen-runs.php` is the instructive one: it does pass options, `['class' => 'border border-dashed ...']`, just not `heading_tag`, so an opts array that is not empty still hit the unset key. The fourteen call sites that passed a real tag were never affected, which is why this survived: the component looked correct everywhere it was tested with an explicit heading.
- Accessibility consequence, not only cosmetic: `aria-labelledby` named `okv-empty-6d2ad963` and no element carried that id, so a screen reader announced the section as unlabelled and read the attribute text as content. The heading element is also the one thing that made the block a landmark with a name.
- Scan for the bug class across the shipped tree. The dangerous shape is a strict whitelist that checks a defaulted read and then casts the raw key in the branch the default just approved. It can only bite when the default is non-empty, because `''` never passes a strict whitelist and the raw read never runs. Checked and safe: `ProOrders::list()` (three filters, all `?? ''`), `Credit` facility state, `ContactMessages` source, `Basket` combo image, `Delivery` cutoff, `admin/content.php` email, and `PaymentHealth.php:124` (`?? 0`, guarded by `> 0`). The only other dynamic tag name in the tree is `ContentRenderer` (`'<h' . $level`), where `$level` is `strlen()` of a matched `##` or `###` and can only be 2 or 3. No stale copy of the broken markup exists anywhere else, and nothing in `assets/` or the JS bundles depends on the heading tag name.
- Tests: new `scripts/tests/EmptyStateTest.php`, 73 assertions under the unit runner. It renders the component and holds the markup to account: a real `h2` with no options, no `< id=`, no attribute text, the closing tag matching the opening one, `aria-labelledby` equal to the id the heading actually carries, the derived id still the heading fingerprint, every whitelisted tag honoured, nine off-whitelist values (including `script`, `H2`, null and an int) falling back to `h2` without a warning, a padded tag trimmed rather than rejected, heading and line escaped, action hrefs and labels escaped, the 44px target on both buttons, no button row when there are no actions, an unknown icon drawing nothing, and the caller's id, class and role used as given. The last section walks all 324 shipped PHP files and fails if the bug class appears anywhere again.
- Proof the guard bites: with the fix stashed, the suite goes red at 3,579 of 3,596 with 17 failures in this file, including the repo-wide scan. With the fix restored, green at 3,596 of 3,596 (the baseline before this test was 3,523).
- Verified in this container as far as it allows: no PHP binary here, so the suite and the lints ran under a WebAssembly PHP 8.5 with the repo mounted. All 324 shipped PHP files parse clean, `bash scripts/brand-check.sh` is green on all eight laws, and the unit suite is green. A scratch before/after fixture was rendered from the real component and served locally so the Owner could see the two panels side by side; it lives outside the repository, because a page in the tree that links the stylesheet without `okv_head_meta()` fails brand law 7, and it is not shipped and not committed. Not run here: the database and HTTP suites, and a browser pass over the live basket, shop, combos, kitchen runs and FAQ pages, which need the MySQL 8 release box.
- One thing left alone on purpose: `kitchen-runs.php` and `shop_results.php` now sit their empty state at `h2` inside a section that already has an `h2`, so the outline would read better with an explicit `'heading_tag' => 'h3'`. That is a document-outline preference, not this defect, and it changes no rendering, so it is noted here rather than folded into a bug fix.
- Merged with `main` at `caa1ac4` (PR #74, the admin catalogue settings modal) before opening the pull request. One conflict, in this file only: both branches appended a 23 September entry, so both are kept, PR #74's first because it shipped first. No code overlapped, the component fix is untouched by it, and the merged tree re-verified: 3,629 of 3,629 assertions green (3,556 from `main` plus the 73 here), brand gate green, all 327 shipped PHP files parse clean.
