...
**Milestone M10, Make It Right, customer reporting in progress on branch `M9-issue-report-make-it-right`.** The customer can report a fixed category, a 10 to 1,000 character description and up to 5 optional JPEG, PNG or WebP photos from an eligible order they own, within the editable 7-day window. The shared upload path checks the original extension, sniffed MIME, configured 5MB default, dimensions and format completeness, gives every file a random name, and rolls back both the report and request-owned files when any photo or database write fails. Issue evidence is denied under `uploads/issues/` and served through an owner-or-`issues.view` route; the owner sees their photos, authorised staff get accessible lazy thumbnails, and the public Order Trail reveals none of it. One active report per order is enforced in MySQL, every creation is audited, attempts are rate-limited, and committed reports notify both the customer and staff. Migration `045` applied to a fresh MySQL 8 database, reapplied with nothing pending, and returned `OK` in the full migration status. Unit tests pass at 2,870/2,870, `issue_reports_db_test.php` passes 24/24, `issue_reports_http_test.php` passes 41/41, and `issue_photos_http_test.php` passes 39/39. The admin resolution workspace, refunds, credits, replacements, resolved outcomes and the browser pass remain open, so the combined M10 acceptance criterion is not ticked.
