...
- [ ] **Task B, business Dashboard, implemented and locally verified. Live verification remains blocked by the existing `/migrations/` and `/docs/` denial failures.** `/pro/index.php` now shows the signed-in business only: open credit outstanding, earliest unpaid due date, overdue amount, available limit, the newest 5 orders with customer-facing lifecycle labels, the next delivery date from `Delivery`, and the 3 most recently updated saved lists when they exist. Empty states link to Credit, My Kitchen Lists and the shop without implying approval or data. The page adds no credit writes and stores no copied balance or delivery rule.
