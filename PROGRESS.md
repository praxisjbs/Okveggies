...
**Milestone M10, Make It Right, workflow in progress on branch `M9-issue-report-make-it-right`.** Customer reporting and private photos are complete. The staff workspace now has an oldest-first active queue, status, category and received-date filters, 25-row pagination, full order and delivery context, verified paid amount, protected photo links, two-way order links, ownership taking, terminal decline, immutable history and audit entries. Reads require `issues.view`; take and decline require both `issues.view` and `issues.resolve`. Migration `046` applied and reapplied cleanly on fresh MySQL 8. Unit tests pass at 2,896/2,896; focused Task C tests pass at 32/32 database and 38/38 HTTP assertions; A/B regressions remain green at 24/24, 41/41 and 39/39; order lifecycle is 11/11 and notifications are 144/144 through local Mailpit. The focused browser pass is green at 390px and 1440px after rebuilding the CSS exposed and fixed a missing desktop split-grid rule. Refund, credit and replacement execution remains deliberately disabled until Task D connects the existing engines; the broader customer terminal-outcome and final role pass remain open.
