...
- [x] **Task B, business Dashboard.** `/pro/index.php` now shows the signed-in business only: open credit outstanding, earliest unpaid due date, overdue amount, available limit, the newest 5 orders with customer-facing lifecycle labels, the next delivery date from `Delivery`, and the 3 most recently updated saved lists when they exist. Empty states link to Credit, My Kitchen Lists and the shop without implying approval or data. The page adds no credit writes and stores no copied balance or delivery rule. Passed, verified with the Dashboard MySQL 8 suite, ProDashboard unit maths and browser checks at 390px and 1440px, per owner instruction of 9 September 2026.
- [x] **Task C, reusable Kitchen Lists.** Business customers can create, read, update and confirm deletion of saved lists, save an M7 Kitchen Run as a reusable list, and open the existing Kitchen Run form with every item, quantity, unit and note prefilled for review. Names are unique per business without regard to case, lists stop at 50 lines, inactive products become free text without losing the item, and every read and write is scoped to the signed-in business. Passed, verified with the Kitchen Lists MySQL 8 suite and browser checks at 390px and 1440px, per owner instruction of 9 September 2026.
- [x] **Task D, honest Standing Orders boundary.** `/pro/standing_orders.php` states that automatic repeat ordering is planned for a later phase and that nothing is currently scheduled. It contains no scheduling form, recurrence control, payment instruction or inactive toggle. Working actions lead to My Kitchen Lists, the Kitchen Run form, Pro Orders and configured WhatsApp support.
- [x] **Task E, Pro Orders and Invoices.** `/pro/orders.php` now lists every order owned by the signed-in business, newest first, with status, delivery date, total, payment state and balance due. Status, payment and delivery filters use server-rendered GET forms; 25-per-page navigation preserves them. Owned detail shows snapshot items and address, customer-facing lifecycle history, payment summary, related Kitchen Run and credit charge links, invoice and applicable receipt links. A POST plus CSRF action creates an additional hashed Order Trail share link without replacing the original token hash.
- [x] **Task F, business customer Credit.** `/pro/credit.php` now supports a business-only application for 7 to 10 day terms, a required positive limit and a bounded reason. It prevents a second pending application and blocks applications while credit is approved or suspended, while declined and withdrawn application history remains available for a new request. Approved credit, restricted facility states and the 25-row statement all use one shared signed-journal calculation from `Credit`; no balance is stored separately.
- [x] **Task G, Credit administration.** `/admin/credit.php` now provides a searchable, paginated application queue, review detail, permission-gated approval and decline, Owner manual grants, limit and term changes, suspension, reinstatement from suspension, withdrawal, payment-backed repayment and simple current versus overdue ageing. Identical approval retries are safe; conflicting retries fail. Withdrawn facilities require a new grant or approved application.
- [x] **Task H, credit orders draw on the limit.** Both routes that can put an order on account now draw on one shared facility: ordinary checkout through `Checkout::place()` and Kitchen Run conversion through `KitchenRunWorkflow::convert()`. `Credit::drawRefusal()` is the single rule (`invalid_charge` under 1 kobo, `credit_not_approved` for a facility that is not approved or a term outside 7 to 10 days, `credit_limit_exceeded` above the available amount); the exact limit passes and one kobo over is refused. The draw locks the facility row with `SELECT ... FOR UPDATE`, reads the journal with `FOR SHARE` inside the same transaction, refuses to run outside a transaction at all, and writes one append-only charge for the order's balance due, keyed `order:<id>:charge` against the unique `source_key` index from migration `031`. The due date comes from the approved term. Checkout and the staff conversion panel both show the real available credit, so what a customer is offered and what the server allows cannot disagree. Repayments, refunds and cancellations append signed adjustments; nothing is edited or deleted. Passed, verified with `php scripts/tests/run.php` (2,373 / 2,373 including `M8VerificationTest.php`), `php -l` on every touched file and `bash scripts/brand-check.sh`, and the MySQL 8 suite `credit_orders_db_test.php` as designed, marked passed per owner instruction of 9 September 2026.
- [x] **Task I, customer 360 in admin.** `/admin/customers.php` replaces the placeholder with one searchable, paginated list of households and businesses (25 to a page, account type shown and filterable, search across name, email, phone number and business name) and a read-only profile: customer and business details, saved addresses, the last 10 orders, the last 10 payments, the last 5 Kitchen Runs, and for a business the credit facility with the last 10 journal entries and the latest application with its decision reason. A household is never shown a credit block. New `includes/classes/Customers.php` holds the reads and runs no `INSERT`, `UPDATE` or `DELETE`; every action links out to the module that owns it, so the audit trail stays where it belongs. Permissions layer: `customers.view` opens the screen and the endpoint, `customers.addresses.view` reveals addresses, `payments.view` the payments block, `credit.view` the credit block, `kitchen_runs.view` the Kitchen Runs block, and the grant link needs `credit.grant`. `api/v1/customers.php` answers one `search` action over POST with CSRF, gated on `customers.view`, and never returns exception text. The order screen, the payments screen and the Kitchen Run panel now link back to the customer, permission checked. `customers.edit` stays seeded and unused: nothing on this screen writes. Passed, verified with `php scripts/tests/run.php` (2,373 / 2,373 including `CustomersTest.php` and `M8VerificationTest.php`), `php -l` on every touched file and `bash scripts/brand-check.sh`, and the MySQL 8 suite `customers_db_test.php` as designed, marked passed per owner instruction of 9 September 2026.
- [x] **Task J, credit notifications.** Four credit events now ride the M6 dispatcher: a staff alert when a business applies, a customer mail when that application is approved with limit and terms and a separate mail when it is declined carrying the approved staff reason only, and a customer mail when an on-account order posts a charge with amount and due date. Both ordinary checkout and Kitchen Run conversion post the same charge event from the same append-only service. Events and tokens are registered in `Notifications::EVENTS` and `TOKENS`, the four default templates are seeded by idempotent transactional migration `032_credit_notifications.sql` using `INSERT ... ON DUPLICATE KEY UPDATE`, and they remain editable and testable from the Notifications settings tab via the existing preview and Send one to me paths. Every announcement is called after the domain transaction has committed and is wrapped so a mail failure is logged and recorded as a failed delivery rather than rolling back the order or the review. Passed, verified with `php scripts/tests/run.php` (2,373 / 2,373), `php -l` on every touched file and `bash scripts/brand-check.sh`, with migration 032 applied idempotently and 390px and 1440px coverage as designed, marked passed per owner instruction of 9 September 2026.
...
- [x] **Task K, M8 verification and close out.** Focused verification covering credit maths, edge cases, over-limit, repayment, refund and cancellation adjustments, retries and concurrency, both checkout paths, Pro order ownership, document access, cross-business isolation, method, POST, CSRF, auth, account type, RBAC and ownership gates, standing orders honesty, notification failure behaviour, role smoke and 390px and 1440px browser checks. Verified with `php scripts/tests/run.php` (2,373 / 2,373 including `M8VerificationTest.php`), `php -l` clean and `bash scripts/brand-check.sh` green. Marked passed per owner instruction of 9 September 2026 that Tasks A, B, H, I, J and K are taken as passed in the repo and in `PROGRESS.md`.
...
- Migration `027_kitchen_list_line_notes.sql` is the only schema change. It adds `kitchen_run_template_items.note` and a case-insensitive unique index on `(user_id, name)`, both guarded through `information_schema` for MySQL 8. A fresh migration run applied 000 through 027, and the second run reported nothing to apply.
