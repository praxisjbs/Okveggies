...
**Homepage hero redesign, review in progress on `arena/01a0bf68-okveggies` (20 September 2026).** The approved produce photograph now fills a full viewport below the unchanged header, with a full forest wash, unboxed copy, the existing seal and tagline, and the existing CMS CTA labels and destinations. The exact approved heading and introduction take display precedence by the Owner's confirmation without rewriting stored CMS copy. The existing motion controller owns the cinematic image entrance, return-to-hero replay, 7% parallax, seal, words, hairline and CTAs; this hero has a final still state for reduced motion and no JavaScript. Local static gates and the browser template fixture pass. PHP-backed integration remains unverified locally, so this is a draft review, not a completion claim for M12, PR6 or PR9. See the session entry below.
