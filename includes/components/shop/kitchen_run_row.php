...
<div data-kr-row class="rounded-[14px] border border-ink-10 bg-white p-3">
  ...
  <input class="okv-input mt-2 min-h-[44px] rounded-xl" id="kr-name-<?= (int) $row ?>" name="items[<?= (int) $row ?>][item_name]" data-kr-name placeholder="Tomatoes, pomo, oil" maxlength="200" value="<?= okv_e((string) ($prefillLine['item_name'] ?? '')) ?>"<?= $disabledAttr ?>>
  <div class="mt-2 grid grid-cols-2 gap-2">
    <div>
      <label class="sr-only" for="kr-qty-<?= (int) $row ?>">How much</label>
      <input class="okv-input min-h-[44px] rounded-xl" id="kr-qty-<?= (int) $row ?>" name="items[<?= (int) $row ?>][quantity]" data-kr-qty inputmode="decimal" placeholder="How much" value="<?= okv_e(($prefillLine['quantity'] ?? null) === null ? '' : okv_quantity($prefillLine['quantity'])) ?>"<?= $disabledAttr ?>>
    </div>
    <div>
      <label class="sr-only" for="kr-unit-<?= (int) $row ?>">Unit</label>
      <select class="okv-input min-h-[44px] rounded-xl" id="kr-unit-<?= (int) $row ?>" name="items[<?= (int) $row ?>][unit_id]" data-kr-unit<?= $disabledAttr ?>>
  ...
  <div class="mt-2" data-kr-price-field <?= $showPrice ? '' : 'hidden' ?>>
    ...
    <input class="okv-input mt-1 min-h-[44px] rounded-xl" id="kr-price-<?= (int) $row ?>" name="items[<?= (int) $row ?>][unit_price]" data-kr-price inputmode="decimal" placeholder="4,000"<?= $disabled || !$showPrice ? ' disabled' : '' ?>>
  </div>
  <div class="mt-2">
    <label class="sr-only" for="kr-note-<?= (int) $row ?>">Note on this item, optional</label>
    <input class="okv-input min-h-[44px] rounded-xl" id="kr-note-<?= (int) $row ?>" name="items[<?= (int) $row ?>][note]" data-kr-note maxlength="255" placeholder="Note, optional. Firm ones please." value="<?= okv_e((string) ($prefillLine['note'] ?? '')) ?>"<?= $disabledAttr ?>>
  </div>
</div>
