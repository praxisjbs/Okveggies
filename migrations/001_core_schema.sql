-- =============================================================================
-- 001_core_schema.sql
-- OK Veggies core schema. Backbone is the client-approved ok_veggies_schema.sql,
-- extended with the PRD Section 20.2 additions (RBAC catalogue, delivery zones,
-- kitchen runs, pairings, OTP, contact, Make It Right, content, counters,
-- rate limits). Every table is CREATE TABLE IF NOT EXISTS, so this is safe to
-- re-run. Foreign key checks are disabled for the whole file and re-enabled at
-- the end, exactly as the drafted schema intended.
-- Money is stored in subunits (kobo) as BIGINT. Never a float.
-- =============================================================================
SET NAMES utf8mb4;
...
-- =============================================================================
-- OK Veggies additions to the drafted schema (PRD Section 20.2).
-- All CREATE TABLE IF NOT EXISTS. Foreign key checks are still 0 here.
-- =============================================================================

-- RBAC permission catalogue and the role/permission junction. Roles and
-- user_roles already exist in the drafted schema above.
...
-- Admin-managed Lagos delivery zones. Captured on the order, shown on the manifest.
CREATE TABLE IF NOT EXISTS `delivery_zones` (
  ...
  `updated_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uq_delivery_zones_slug` (`slug`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- Kitchen Runs: a line-item request we quote, the customer approves, it becomes an order.
CREATE TABLE IF NOT EXISTS `kitchen_run_requests` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  ...
  `pricing_mode` VARCHAR(20) NOT NULL DEFAULT 'by_us',
  `status` VARCHAR(30) NOT NULL DEFAULT 'submitted',
  `is_open_budget` BOOLEAN NOT NULL DEFAULT FALSE,
  `budget_ceiling_subunit` BIGINT UNSIGNED NULL,
  `spend_cap_subunit` BIGINT UNSIGNED NULL,
  `estimated_total_subunit` BIGINT UNSIGNED NULL,
  `quoted_total_subunit` BIGINT UNSIGNED NULL,
  `deposit_subunit` BIGINT UNSIGNED NULL,
  ...
  `updated_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uq_kitchen_run_requests_request_number` (`request_number`),
  CONSTRAINT `fk_kitchen_run_requests_user_id` FOREIGN KEY (`user_id`) REFERENCES `users` (`id`) ON DELETE SET NULL ON UPDATE CASCADE,
  CONSTRAINT `fk_kitchen_run_requests_zone` FOREIGN KEY (`delivery_zone_id`) REFERENCES `delivery_zones` (`id`) ON DELETE SET NULL ON UPDATE CASCADE,
  CONSTRAINT `fk_kitchen_run_requests_order` FOREIGN KEY (`converted_order_id`) REFERENCES `orders` (`id`) ON DELETE SET NULL ON UPDATE CASCADE,
  CONSTRAINT `fk_kitchen_run_requests_quoted_by` FOREIGN KEY (`quoted_by`) REFERENCES `users` (`id`) ON DELETE SET NULL ON UPDATE CASCADE,
  INDEX (`status`, `created_at`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

CREATE TABLE IF NOT EXISTS `kitchen_run_items` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  ...
  `unit_price_subunit` BIGINT UNSIGNED NULL,
  `line_total_subunit` BIGINT UNSIGNED NULL,
  `price_source` VARCHAR(20) NOT NULL DEFAULT 'admin',
  `sort_order` INT UNSIGNED NOT NULL DEFAULT 0,
  `note` VARCHAR(255) NULL,
  `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  CONSTRAINT `fk_kitchen_run_items_request` FOREIGN KEY (`request_id`) REFERENCES `kitchen_run_requests` (`id`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `fk_kitchen_run_items_product` FOREIGN KEY (`product_id`) REFERENCES `products` (`id`) ON DELETE SET NULL ON UPDATE CASCADE,
  CONSTRAINT `fk_kitchen_run_items_unit` FOREIGN KEY (`unit_id`) REFERENCES `units_of_measurement` (`id`) ON DELETE SET NULL ON UPDATE CASCADE,
  INDEX (`request_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- Saved, reusable B2B kitchen lists for the Pro Portal.
CREATE TABLE IF NOT EXISTS `kitchen_run_templates` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  `user_id` BIGINT UNSIGNED NOT NULL,
  ...
  `updated_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  CONSTRAINT `fk_kitchen_run_templates_user` FOREIGN KEY (`user_id`) REFERENCES `users` (`id`) ON DELETE CASCADE ON UPDATE CASCADE,
  INDEX (`user_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

CREATE TABLE IF NOT EXISTS `kitchen_run_template_items` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  ...
  `sort_order` INT UNSIGNED NOT NULL DEFAULT 0,
  PRIMARY KEY (`id`),
  CONSTRAINT `fk_krti_template` FOREIGN KEY (`template_id`) REFERENCES `kitchen_run_templates` (`id`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `fk_krti_product` FOREIGN KEY (`product_id`) REFERENCES `products` (`id`) ON DELETE SET NULL ON UPDATE CASCADE,
  CONSTRAINT `fk_krti_unit` FOREIGN KEY (`unit_id`) REFERENCES `units_of_measurement` (`id`) ON DELETE SET NULL ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- "Goes well with" pairings shown on the product page.
CREATE TABLE IF NOT EXISTS `product_pairings` (
  ...
  `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uq_product_pairings` (`product_id`, `paired_product_id`),
  CONSTRAINT `fk_product_pairings_product` FOREIGN KEY (`product_id`) REFERENCES `products` (`id`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `fk_product_pairings_paired` FOREIGN KEY (`paired_product_id`) REFERENCES `products` (`id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- One-time codes for account activation (email now, SMS later). Required before pay-on-delivery.
CREATE TABLE IF NOT EXISTS `otp_verifications` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  `user_id` BIGINT UNSIGNED NULL,
  `identifier` VARCHAR(255) NOT NULL,
  `channel` VARCHAR(20) NOT NULL DEFAULT 'email',
  `purpose` VARCHAR(40) NOT NULL DEFAULT 'account_activation',
...
-- Storefront contact-form submissions surfaced in the admin panel.
CREATE TABLE IF NOT EXISTS `contact_messages` (
  ...
  `source` VARCHAR(40) NOT NULL DEFAULT 'storefront_contact_form',
...
-- Make It Right: a customer reports an order problem, admin resolves it.
CREATE TABLE IF NOT EXISTS `issue_reports` (
  ...
  `resolved_at` DATETIME NULL,
  PRIMARY KEY (`id`),
  CONSTRAINT `fk_issue_reports_order` FOREIGN KEY (`order_id`) REFERENCES `orders` (`id`) ON DELETE RESTRICT ON UPDATE CASCADE,
  CONSTRAINT `fk_issue_reports_user` FOREIGN KEY (`user_id`) REFERENCES `users` (`id`) ON DELETE SET NULL ON UPDATE CASCADE,
  CONSTRAINT `fk_issue_reports_handled_by` FOREIGN KEY (`handled_by`) REFERENCES `users` (`id`) ON DELETE SET NULL ON UPDATE CASCADE,
  INDEX (`status`, `created_at`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

CREATE TABLE IF NOT EXISTS `issue_report_photos` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  `issue_id` BIGINT UNSIGNED NOT NULL,
  `photo_url` VARCHAR(500) NOT NULL,
  `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  CONSTRAINT `fk_issue_report_photos_issue` FOREIGN KEY (`issue_id`) REFERENCES `issue_reports` (`id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- Editable copy for About, How It Works, FAQ and legal pages.
CREATE TABLE IF NOT EXISTS `content_pages` (
  ...
  CONSTRAINT `fk_content_pages_updated_by` FOREIGN KEY (`updated_by`) REFERENCES `users` (`id`) ON DELETE SET NULL ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- Atomic sequences (order numbers, kitchen-run numbers).
...
-- Per-bucket rate limiting for auth, OTP and password endpoints.
