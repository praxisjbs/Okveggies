...
try{
 if($action==='apply'){
  Customer::requireLoginApi();if(!Customer::isBusiness()){okv_error('This action is for business accounts.',403,'forbidden');}
 ...
 Rbac::requirePermission($permissions[$action]);$staffId=(int)Rbac::userId();$entityId=0;$result=[];
 if($action==='approve'){$entityId=(int)okv_input('application_id',0);$result=Credit::approveApplication($entityId,$staffId,okv_input('credit_days',''),okv_input('credit_limit',''));}
 elseif($action==='decline'){$entityId=(int)okv_input('application_id',0);$result=Credit::declineApplication($entityId,$staffId,(string)okv_input('decision_reason',''));}
 elseif($action==='grant'){$entityId=(int)okv_input('business_id',0);$result=Credit::grant($entityId,$staffId,okv_input('credit_days',''),okv_input('credit_limit',''));}
 elseif($action==='change_terms'){$entityId=(int)okv_input('business_id',0);Credit::changeTerms($entityId,okv_input('credit_days',''),okv_input('credit_limit',''));}
 elseif(in_array($action,['suspend','withdraw','reinstate'],true)){$entityId=(int)okv_input('business_id',0);Credit::transitionFacility($entityId,['suspend'=>'suspended','withdraw'=>'withdrawn','reinstate'=>'approved'][$action]);}
 else{$entityId=(int)okv_input('business_id',0);$result=Credit::recordRepayment($entityId,(int)okv_input('payment_id',0));}
