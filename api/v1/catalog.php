...
// Every action below reads the catalogue. One guard keeps a database failure
// a JSON 500 the storefront can handle, never a blank fatal the live search
// cannot parse.
try {
    switch ($action) {
        case 'categories':
    ...
    error_log('catalog.' . ($action !== '' ? $action : 'unknown') . ' failed: ' . $e->getMessage());
    okv_error('Something went wrong at our end. Please try again.', 500, 'failed');
}
